...
**Scale at time of writing:** 284 migrations, 468+ tables (`compliance` schema), 212 files in `src/lib/services/`, 995+ API route files, 188+ page files, ~225K lines of TS/TSX (re-verified 2026-08-07, task-20260807-071602 -- see `scripts/check-doc-scale-freshness.mjs`, which parses this exact line and fails if the live counts drift >20% from it; original snapshot was 115/460+/114/573+/145+/~61K as of 2026-07-09, and had drifted 30-147% by the time this correction landed. **Correction (independent audit, PR #1047):** the script exists, passes locally, and is designed to be a CI check, but is **not yet wired into `.github/workflows/ci.yml`** -- see that script's own header and `PROGRESS.md` for why (blocked on this session's `gh` token lacking the `workflow` OAuth scope needed to push a workflow-file change). Until that wiring lands, this line is not yet actually enforced by CI -- treat it as a documented, tested, ready-to-wire check, not a live gate). PROJEXA (construction ERP) lives inside this same repo, not a separate one.
