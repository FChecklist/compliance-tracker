...
**VCEL (computation engines)** — 32 files in `src/lib/engines/` as of 2026-08-07 (25 at the time this section was originally written), tracked in a `computationEngines` registry table (`status`: implemented/partial/not_started). Dispatch is a deliberately small, explicit `switch` (`dispatchEngine()`), never a generic resolver that dynamic-imports `implementationRef` — that would be a real code-execution injection surface. **Corrected 2026-08-01, re-verified 2026-08-07**: the "~15 of 25 wired" figure here is stale — `docs/master/CAPABILITY_COVERAGE.md` put it at ~160-170 wired engineKeys as of its last (2026-07-18/19) regeneration, itself already flagged there as outdated relative to later waves. Coverage percentage is a moving target and not restated here — re-run that doc's own SQL query against `compliance.computation_engines` for the true current number rather than trusting any fixed figure, including this one.
