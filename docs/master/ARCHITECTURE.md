...
**VCEL (computation engines)** — 32 files in `src/lib/engines/` (re-verified 2026-08-07, task-20260807-071602; was 25 at the time this section was originally written), tracked in a `computationEngines` registry table (`status`: implemented/partial/not_started). Dispatch is a deliberately small, explicit `switch` (`dispatchEngine()`), never a generic resolver that dynamic-imports `implementationRef` — that would be a real code-execution injection surface. **Corrected 2026-08-07**: the "~15 of 25 wired" figure here was stale; a direct re-count of `dispatchEngine()`'s live switch in `src/lib/task-execution-engine.ts` finds 190 real `case` branches (191 dispatchable `engineKey`s counting the GST special-case) — this is a re-verified line count, not the `computationEngines` registry's own category/percentage breakdown, which still needs a live DB query to state accurately (see `docs/master/CAPABILITY_COVERAGE.md`, itself already flagged there as stale relative to later waves — re-run its own SQL query against `compliance.computation_engines` for the true current number rather than trusting any fixed percentage, including this one).
