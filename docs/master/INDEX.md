...
| [`docs/master/API_CHANGELOG.md`](API_CHANGELOG.md) | 🆕 | Public `/api/v1/*` changelog (added 2026-07-16, Review Framework Wave A) — real dated entries built from commit history, not a placeholder. Names a concrete, confirmed gap: ~90 `/projexa/*` PM-ERP paths added 2026-07-13/14 are live but not yet registered in the public OpenAPI spec (`src/lib/openapi/generate.ts`). |
| [`docs/master/SEV1_INCIDENT_RUNBOOK.md`](SEV1_INCIDENT_RUNBOOK.md) | 🆕 | First internal Sev-1 incident-command runbook (added 2026-07-16, Review Framework Wave A) — real roles, detection paths, and a Vercel-deployment-rollback-first procedure grounded in the actual project (`compliance-tracker-ai`, `prj_mRRWcMvhyuxgRZtcfp4ArSzcOvII`) and the documented CI/auto-deploy race in `AUDIT_2026-07-09.md` §25. Explicitly unrehearsed — closes the audit's "no DR/incident practice exists internally" finding as a first pass, not a proven procedure. |
