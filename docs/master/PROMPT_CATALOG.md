...
Re-verified 2026-08-07 (task-20260807-071614): re-scanned `src/` for
`resolvePromptTemplate('...')` call sites independently — all 26 keys below
still match exactly, no new call sites added or removed in the intervening
6 days.

## What already exists (real, live code)

- **Schema**: `compliance.promptTemplates` / `compliance.promptVersions`
  (`src/lib/db/schema.ts:2182-2247`, re-verified 2026-08-07 — was
  2198-2235 at 2026-08-01, drifted with unrelated upstream edits) — each
  version has `content`, `label`
...
**Keeping this table current**: it is a snapshot from a grep pass
(originally 2026-08-01, re-verified 2026-08-07), not a generated artifact —
a future `resolvePromptTemplate()` call site added without updating this
table will silently drift, same staleness risk as
`docs/master/MODULE_MAP.md` before `scripts/check-doc-scale-freshness.mjs`
existed (see that script and `ai-os/registry/business-rules-registry.yaml`'s
own header for the same honest caveat). Not CI-enforced this pass — a
future extension could grep `resolvePromptTemplate\(['"]` and diff against
