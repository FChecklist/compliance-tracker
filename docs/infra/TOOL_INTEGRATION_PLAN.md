...
- **T0.4** `[SUPERVISOR]` **DONE 2026-07-10.** `.github/workflows/doc-processing-job.yml` written: triggered by `repository_dispatch` (event type `doc-processing-job`, payload `{job_id, operation, input}`), pulls the ghcr.io image, waits for `/health`, POSTs to `/${operation}`, writes `status`/`result`/`error` back to `compliance.doc_processing_jobs` (migration `0133`, applied live) via the Supabase REST API using the service role key. Refreshed the `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` GitHub secrets as part of this (previously flagged stale in `scripts/ai-workforce-agent.mjs`'s comments — confirmed current against `.env.local`, now safe to use directly instead of routing around them). **Not yet end-to-end tested** — the container only has a `/health` endpoint until Phase 1's `T1.1` adds `/ocr`; this workflow's logic is complete and ready to receive real dispatches once that lands.
...
- **Phases 1, 2, 4, 5** (PaddleOCR, Docling, Whisper.cpp, LibreOffice): T0.4 done — ready to dispatch T1.1 (`/ocr` endpoint, highest priority, confirmed gap) next.
