...
**3. "Create similar task again"** — once a chain is complete and text is typed, this button dispatches immediately against the *same* selected chain and clears only the typed message, ready for the next instruction against that identical chain right away (`createSimilarTaskAgain()`). Superseded the earlier stage-and-reset queue mechanism (VERI_CHAT_MOCKUP_TO_PRODUCTION_SPEC_2026-08-01.md §3.2.5, Owner: "must actually create another task using the SAME chain, not stage-and-reset") — `queueCurrent()`/`sendAllQueued()` and the queue UI were removed entirely, not left disabled.
...
| Queue / "+ Add another" / "Send all" | **Removed 2026-08-01** (VERI_CHAT_MOCKUP_TO_PRODUCTION_SPEC_2026-08-01.md §3.2.5) — replaced by "Create similar task again" (`createSimilarTaskAgain()` in `VeriComposer.tsx`), which dispatches immediately against the same chain instead of staging+resetting it. |
