...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ clientId: string }> }) {
  ...
  try {
    ...
    const taxCases = await listTaxCasesForClient({ orgId, userId: dbUser.id, dbUser }, clientId)
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ clientId: string }> }) {
  ...
  try {
    ...
    const taxCase = await createTaxCase({ orgId, userId: dbUser.id, dbUser }, { ...body, clientId })
