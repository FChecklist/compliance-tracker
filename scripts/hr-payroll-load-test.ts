...
async function timeCalls<T>(label: string, iterations: number, fn: () => Promise<T>): Promise<{ label: string; iterations: number; p50Ms: number; p95Ms: number; maxMs: number; errors: number }> {
