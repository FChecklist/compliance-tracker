...
const RULES = [
  ...
  // --- Rebase-time addition (2026-08-31): tables added to schema.ts on
  // main since this script/doc were first written (431 -> 475 tables) that
  // fell through to "uncategorized" on re-run. Each verified against its
  // actual schema.ts definition/comment, same as every rule above, not
  // guessed from the table name alone.
  { test: n => ['public_ticket_submission_attempts', 'ticket_teams', 'business_hours_schedules', 'sla_policies', 'escalation_rules', 'ticket_escalation_events'].includes(n), category: 'universal', note: 'generic customer-service/ticketing infra (rate-limit log, teams, business-hours SLA/escalation config) -- same ITIL-shaped domain as tickets/installed_products above, no India-specific rule' },
  { test: n => n === 'tenant_ai_config', category: 'universal', note: 'per-org BYO AI model config (provider/key/model for the software_team dispatch path) -- AI orchestration platform layer, product-agnostic' },
  { test: n => n === 'business_terminology_glossary', category: 'universal', note: 'generic AI-explainability term glossary -- category column is explicitly free-text spanning finance/compliance/construction/crm, cross-domain by design' },
  { test: n => n.startsWith('hr_expense_claims') || n.startsWith('hr_employee_loans') || n === 'hr_loan_installments' || n === 'hr_shift_types' || n === 'hr_shift_roster_assignments', category: 'universal', note: 'generic HR (expense reimbursement, employee loans, shift rostering) -- no statutory/regulatory regime attached, distinct from the hr_compliance_items/erp_statutory_* compliance rules above' },
  { test: n => n === 'abac_policies', category: 'universal', note: 'generic deny-only attribute-based access-control overlay on top of RBAC (abac-policy-service.ts) -- platform authorization infra, not a business domain' },
  { test: n => n === 'performance_review_goals' || n === 'performance_review_raters', category: 'universal', note: 'generic performance-appraisal detail tables, same domain as performance_review_cycles/performance_reviews above' },
  { test: n => n === 'calculation_invocations', category: 'universal', note: 'audit log for the generic computation-engine invocation layer, same domain as computation_engines/token_usage_ledger above' },
  { test: n => n.startsWith('business_rule'), category: 'universal', note: 'generic business-rules engine (rule definitions, versions, test runs) -- a rule authoring/execution platform, not itself a business domain' },
  { test: n => n === 'support_sessions', category: 'universal', note: 'act-on-behalf-of-customer support-session capability (support-session-service.ts) -- generic customer-support infra, any org/industry' },
  { test: n => ['submissions', 'pipeline_tasks', 'phrase_map', 'gap_log'].includes(n), category: 'universal', note: 'M24/M25 mode-pill -> chain -> task pipeline (chat composer classify/dispatch engine) -- AI orchestration platform layer, same bucket as the ai_/task*/orchestra_ rule above' },
  { test: n => n === 'screen_definitions' || n === 'screen_drafts', category: 'universal', note: 'M28/M29 dynamic screen registry + draft lifecycle -- generic UI-generation platform infra, not a business domain' },
  { test: n => n === 'reuse_cache' || n === 'incident_log' || n === 'memory_store', category: 'universal', note: 'R46 AI request/response reuse-store (exact-hash + semantic-similarity reuse, working memory) -- platform infra; incident_log here logs reuse-pipeline incidents, not GRC/compliance incidents' },
  { test: n => n === 'pill_usage' || n === 'chain_history', category: 'universal', note: 'M24 pill-ranking + chain-execution history for the chat composer -- AI orchestration platform layer' },
  { test: n => ['source_object', 'document_chunk', 'extraction_profile', 'precedent', 'retrieval_citation', 'chunk_policy', 'crr_erasure_log', 'crr_ingest_error'].includes(n), category: 'universal', note: 'CRR generic document-capture/RAG pipeline (unified capture table replacing the prior India-tax-specific extractor; chunking, citation, and semantic-reuse "precedent" scoring) plus a generic right-to-erasure audit log -- explicitly built to replace India-specific hard-coding with an industry-neutral pipeline, see this section\'s own schema.ts header comment' },
