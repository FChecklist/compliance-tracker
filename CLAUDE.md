...
8. **R70 findings (2026-09-04), not yet fixed — check before relying on any of these:**
   - `compliance.memory_records`'s append-only trigger (R68 Phase 1) blocks `UPDATE` tampering correctly but does **not** block `DELETE` at all — an `app_runtime`-level `DELETE` permanently destroys a record. Verified live via a rolled-back transaction test. High severity, unfixed.
   - `compliance.connector_accounts.composio_connected_account_id` is `NOT NULL`, hard-coupling the table to Composio (a vendor already declined). A direct-OAuth connector row cannot be inserted today. Blocks any direct-Google-OAuth work until the schema changes.
   - `src/lib/embeddings.ts`'s `findSimilar()` on `origin/main` **does** correctly filter by `embedding_model` (R68 Phase 5) — but reading this file from a stale/behind-`origin/main` branch shows the old, unfiltered version with no error. Always check which git ref a code-reading step is actually running against.
   - Migrations 0548–0550 (memory_records location columns, graph_edge validity-bound columns, current-row partial indexes) are applied live and registered, but only committed on branch `r70/windows-path-bugs-fix` (pushed, not merged — `origin/main` doesn't have them in its working tree yet, only in the live DB schema).
   - Both Vercel production apps were paused (`DEPLOYMENT_PAUSED`) as of this date — check current status before assuming either is reachable.
