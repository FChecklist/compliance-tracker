...
8. **R71 state (2026-09-04) — supersedes the R70 findings below it used to list. Check this before relying on the memory/graph store or planning a deploy:**
   - **All 13 `r67/*`/`r70/*` Part-A branches now merged into `main` and pushed** (12 real merges + 1 confirmed-redundant, `r67/e2-reports-charts-2`, already an ancestor of main via PR #1588). 112 conflicts resolved, each with a recorded per-file justification in its own commit message — **never assume a blanket keep-ours/keep-theirs; read the actual commit** if you need to know why a specific file went the way it did. Zero regressions (verified via an A/B test against the pre-merge tag, `git tag r71-premerge-main`).
   - **`memory_records` DELETE gap is FIXED** (migration 0551) — a role-gated `BEFORE DELETE` trigger now blocks any `app_runtime` delete unless `app.memory_delete_authorized` is set by an authorised code path. `memory_versions`/`memory_sources` are now fully insert-only (0552/0553, zero exceptions, not even break-glass). `graph_edge`/`graph_node` gained matching DELETE (and, for `graph_node`, UPDATE) guards (0553) — the platform-tier self-healing reconciliation functions (`graph_full_resync`/`graph_reconcile_platform_tier`) are `SECURITY DEFINER` owned by `postgres`, not `app_runtime`, so these guards do not affect them.
   - **A real, more severe, pre-existing bug was found and fixed in the OLDER append-only guard** (migration 0554): `memory_records.search_vector` is a `GENERATED` column, and Postgres presents it as `NULL` in `NEW` inside a `BEFORE UPDATE` trigger (computed only after `BEFORE` triggers return). The guard never excluded it, so **every legitimate update via `app_runtime` — including the one supersede path the guard exists to still allow — has been silently rejected since this guard's creation at R68 Phase 1.** If you were ever told "the supersede path works, it's proven," it wasn't, until this fix.
   - `compliance.connector_accounts.composio_connected_account_id` is still `NOT NULL` — the Composio coupling is unchanged, still blocks any direct-Google-OAuth connector row.
   - `memory_records` now has a real dedup constraint (`memory_records_tenant_scoped_content_unique`, migration 0555, functional index on tenant+scope+content_hash, NULLs normalised) and three new axes: `engagement_id` (nullable, references the new `compliance.engagements` table), `subject_class` (`NOT NULL DEFAULT 'UNRESOLVED'`, six values), `record_class` (nullable, seven values, distinct from `memory_type`).
   - 143 `embedding_model='unknown-legacy'` rows are now marked `quarantined=true` (migration 0559) — never deleted, never re-embedded, excluded from future comparison.
   - `vercel.json` restricts deployments to `main` only (`{"*": false, "main": true}`) — **verified in behaviour**, not just the file: zero non-main-branch deployments created since that restriction's commit. Both production aliases are still `DEPLOYMENT_PAUSED` (503) as of R71's close — check current status before assuming either is reachable; a real deploy at this point would ship 198 commits behind zero CI/preview verification, Phase 4's local gate (install/typecheck/test/lint/build, all green) is the only thing that has ever run against this commit set.
   - `drizzle/meta/_journal.json` was already 5 migrations behind the real files (0546–0550 unregistered) before R71 touched it — not fixed, flagged for a future reconciliation pass, don't assume the journal is complete.
   - 34 owner decisions are open (`platform.img_spec where blocked_on_human`) — full register in `platform.claude_log` id 185. R70-O-01 (Vercel restore) is first by long-standing convention.
