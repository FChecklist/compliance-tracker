...
- `src/lib/db/schema.ts` (~11,500 lines) — the single biggest blast radius in the repo: every table for every module. A wrong migration or type change here is repo-wide, not local.
- ERP/service cluster, each ~440–600 lines, no colocated tests: `src/lib/services/erp-accounting-service.ts`, `erp-financial-report-service.ts`, `erp-procurement-workflow-service.ts`, `compliance-service.ts`, `activity-log-service.ts`, `veri-reward-service.ts`, `workspace-memory-service.ts`, `veri-meeting-service.ts`.
- Large untested UI components: `src/components/veri-chat/VeriComposer.tsx`, `VeriChatPanel.tsx`, `src/components/AppSidebar.tsx`, `search-command.tsx`, `DocumentUploadSection.tsx`, `InviteUserModal.tsx`, `CustomReportsSection.tsx`, `WebhookSection.tsx`, `AiConfigSection.tsx` — plus `src/components/ui/sidebar.tsx`, which is vendored shadcn/ui, not hand-authored; prefer regenerating/upgrading it via shadcn's CLI over hand-editing.
- API route handlers: `src/app/api/mcp/route.ts` (the MCP tool-call surface) and `src/app/api/ai/orchestrate/route.ts` — untested and large, same "read the whole thing first" caution applies.

This list is not exhaustive (21 files matched the last time this was regenerated — see the script for the current full set) and the ≥400-line/no-test-file bar is a heuristic, not a verdict: a short file with gnarly multi-tenant RLS logic can be riskier than a long mechanical one, and a file can have real indirect coverage this purely-textual check can't see. `src/lib/supabase/auth-guard.ts` (backs `requireAuth()`, used by every API route) no longer appears here as of this writing — it has real colocated tests (`auth-guard.test.ts`) — but stays worth extra care regardless of what this heuristic says, since it is the security boundary for the whole API surface.
