...
// R67 D-17: Delete had no route at all on this surface, which is why the
// PROJEXA object page could only ever HIDE the control. Soft, draft-only, and
// refusing with deleteVeriMeeting()'s own sentence -- the same string the
// disabled button shows -- so the UI never has to invent a reason.
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (error instanceof ServiceError) return NextResponse.json({ error: error.message }, { status: error.status })
    console.error("v1 projexa veri-meeting delete error:", error)
