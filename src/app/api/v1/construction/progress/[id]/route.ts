// R67 D-28 (R-069): a single work-progress entry -- read, correct, delete.
// Work Progress was create-only: listProgressEntries + createProgressEntry and
// nothing else on the wire, so a mis-keyed quantity could only be fixed by a
// DELETE (itself unreachable, no route existed) and a full re-entry. GET/PATCH
// run through the SAME service functions the list and the create path use, so
// the percent range, the entry-basis vocabulary, the project-scoped activity
// lookup and the parent-BOQ-line refusal are one implementation, not two.
//
// Same auth posture as ../route.ts: requireAuthOrApiKey for the read,
// requireRoleOrScope(member/write) for both writes -- PROJEXA reaches these
// with a per-org Bearer key.
