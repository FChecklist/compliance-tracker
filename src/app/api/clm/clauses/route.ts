import { NextResponse } from "next/server"
import { requireAuth, requireRole } from "@/lib/supabase/auth-guard"
import { listClauses, createClause, ServiceError } from "@/lib/services/erp-contract-service"

export async function GET() {
  const { response, orgId } = await requireAuth()
  if (response) return response
  if (!orgId) return NextResponse.json({ clauses: [] })

  const clauses = await listClauses({ orgId })
  return NextResponse.json({ clauses })
}

export async function POST(request: Request) {
  const { response, dbUser, orgId } = await requireAuth()
  if (response) return response
  if (!orgId || !dbUser) return NextResponse.json({ error: "No organisation found" }, { status: 400 })
  const roleCheck = requireRole(dbUser, "manager")
  if (roleCheck) return roleCheck

  try {
    const body = await request.json()
    const clause = await createClause({ orgId, userId: dbUser.id }, body)
    return NextResponse.json(clause, { status: 201 })
  } catch (error) {
    if (error instanceof ServiceError) return NextResponse.json({ error: error.message }, { status: error.status })
    console.error("Clause create error:", error)
    return NextResponse.json({ error: "Failed to create clause" }, { status: 500 })
  }
}
