...
export async function GET(request: NextRequest) {
  ...
  if (!orgId) return NextResponse.json({ notices: [], total: 0, page: 1, limit: 20, totalPages: 0 })
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ id: result.notice.id, noticeNumber: result.notice.noticeNumber, status: result.notice.status }, { status: 201 })
  } catch (error) {
