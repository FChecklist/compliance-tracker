...
export async function GET(_req: NextRequest, ctx: Context) {
  ...
  try {
    ...
    console.error('Ingestion batch fetch error:', error)
    return NextResponse.json({ error: 'Failed to fetch ingestion batch' }, { status: 500 })
  }
}
...
export async function DELETE(_req: NextRequest, ctx: Context) {
  ...
  try {
    ...
    console.error('Ingestion batch cancel error:', error)
    return NextResponse.json({ error: 'Failed to cancel ingestion batch' }, { status: 500 })
  }
}
