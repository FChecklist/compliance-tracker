...
import { db, ingestionBatches, ingestionItems, users, organisations } from '@/lib/db'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!org || !adminUser) {
      return NextResponse.json({ error: 'Organisation not set up' }, { status: 500 })
    }
    ...
    const validated = await validateItems(extracted.items, org.id)
    ...
    if (batchId) {
      ...
      }).where(eq(ingestionBatches.id, batchId)).catch(() => {})
    }
...
// GET /api/ingest — list all batches
export async function GET() {
  const { response } = await requireAuth()
  if (response) return response

  const batches = await db.query.ingestionBatches.findMany({
