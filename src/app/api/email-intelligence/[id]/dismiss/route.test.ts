...
// "member") gate added to POST /api/email-intelligence/[id]/dismiss -- see
// the route's own comment for why "member" was chosen (matched to its twin
// review-decision sibling, ../promote/route.ts).
...
// route only calls dismissEmailIntelligenceItem -- see ../../route.test.ts's
...
function makeRequest(): Request {
  return new Request("http://localhost/api/email-intelligence/eii-1/dismiss", { method: "POST" })
}
...
describe("POST /api/email-intelligence/[id]/dismiss (access control)", () => {
  test("a role below member (viewer) is rejected with 403 and dismissEmailIntelligenceItem is never called", async () => {
    ...
    mock.module("@/lib/services/email-intelligence-service", () => ({
      dismissEmailIntelligenceItem: mock(async () => { throw new Error("dismissEmailIntelligenceItem should not be reached for a below-minimum role") }),
  ...
  test("a member-rank caller is allowed through and the item is dismissed", async () => {
    mock.module("@/lib/supabase/auth-guard", () => ({
      requireAuth: mock(async () => ({ response: null, dbUser: dbUser("member"), orgId: "org-1" })),
      requireRole: fakeRequireRole,
    }))
    mock.module("@/lib/services/email-intelligence-service", () => ({
      dismissEmailIntelligenceItem: mock(async () => ({ id: "eii-1", status: "dismissed" })),
      analyzeInboundEmail: mock(async () => { throw new Error("not under test in this file") }),
      listEmailIntelligenceItems: mock(async () => []),
      promoteEmailIntelligenceItem: mock(async () => { throw new Error("not under test in this file") }),
      sanitizeSuggestedWorkItems: mock(() => []),
