...
import { markObligationNotMyJob } from "@/lib/services/dpdp-obligation-service"
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ obligationId: string }> }) {
  ...
  try {
    ...
    const updated = await markObligationNotMyJob(result.ctx.orgId, result.ctx.identityId, label, obligationId, body?.reason ?? "")
