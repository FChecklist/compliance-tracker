...
import { markIndividualsNotified } from "@/lib/services/dpdp-governance-service"
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ breachId: string }> }) {
  ...
  try {
    ...
    const updated = await markIndividualsNotified(result.ctx.orgId, result.ctx.identityId, breachId)
