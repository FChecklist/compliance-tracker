...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ clientId: string }> }) {
  ...
  try {
    ...
    const engagements = await listEngagementsForClient({ orgId, userId: dbUser.id, dbUser }, clientId)
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ clientId: string }> }) {
  ...
  try {
    ...
    const engagement = await createEngagement({ orgId, userId: dbUser.id, dbUser }, { ...body, clientId })
