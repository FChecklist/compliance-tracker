...
// GAP-OCID038-PROJEXA-DOMAIN-BRAND-MISMATCH addendum (this fix): this file
// used to be the "use client" default export of page.tsx (100% client-side,
// hardcoded "VERIDIAN AI" wordmark), the same real gap independently found
// and fixed on src/app/login/login-form.tsx by the Stage 1 pre-authentication
// domain-based brand resolution work (org-branding-service.ts's
// resolvePreAuthBrandByHost(), PR #886). Split out to its own file (matching
// login/login-form.tsx's own split from login/page.tsx) so page.tsx can
// become an async Server Component -- the only way to read the real HTTP
// Host header before any session exists -- and hand the resolved brand down
// as a plain prop. `brand` is resolved server-side by the parent page.tsx
// (real HTTP Host header -> product_branches lookup) -- this component does
// no I/O of its own, matching login-form.tsx's own "enhance, don't build a
// second engine" shape. `null` (the common case: no host match) means
...
//
// Honest scope note (do not silently widen this beyond what login-form.tsx
// itself actually does): PreAuthBrand only carries `brandName` (and
// `productBranchId`) -- it has no tagline/footer fields, so `tAuth("tagline")`
// and `tAuth("footer")` below stay exactly as they already are on login, not
// brand-resolved. That is consistent with the existing pattern, not a gap
// introduced by this change.
export function SignupForm({ brand }: { brand: PreAuthBrand | null }) {
