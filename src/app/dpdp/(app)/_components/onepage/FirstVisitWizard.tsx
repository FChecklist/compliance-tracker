...
export function FirstVisitWizard({
  orgName, rows, areas, ownerEmail, onComplete,
}: {
  orgName: string
  rows: ObligationRow[]
  areas: Area[]
  ...
  function handleSave() {
    ...
    // Both fixes are needed, found live in that order: (1) await inside the
    // transition, so React tracks the mutation as pending work; (2) an
    // explicit router.refresh() after it resolves -- invoking a server
    // action directly (not through a <form action={...}>) does NOT itself
    // trigger Next to refetch the current route's RSC payload, even though
    // the action's own revalidatePath ran. Without this, the DB write
    // (firstVisitSeenAt, the area assignments) succeeded every time but the
    // page kept showing the wizard until a manual reload.
