...
type Complaint = { id: string; caseRef: string; receivedDate: string; status: string };
...
export default function PoshPage() {
  ...
  const load = () => {
    fetch("/api/posh").then((r) => r.json()).then((d) => {
      ...
      setCommittee(d.committee ?? []); setComplaints(d.complaints ?? []); setAnnualReports(d.annualReports ?? []);
    });
  };
  useEffect(load, []);
    ...
    <div className="space-y-4">
      ...
      <Card className="rounded-xl shadow-card bg-white p-4">
        ...
        <table className="w-full text-xs">
          <thead><tr className="text-left text-ct-muted border-b border-ct-border"><th className="pb-2 font-medium">Case ID</th><th className="pb-2 font-medium">Received</th><th className="pb-2 font-medium">Status</th></tr></thead>
          <tbody className="divide-y divide-ct-border">{complaints.map((c) => <tr key={c.id}><td className="py-2">{c.caseRef}</td><td className="py-2">{new Date(c.receivedDate).toLocaleDateString("en-IN")}</td><td className="py-2"><StatusPill value={c.status} /></td></tr>)}</tbody>
        </table>
      </Card>
    </div>
  );
