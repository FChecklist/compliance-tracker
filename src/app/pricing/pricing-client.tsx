...
// 2026-08-05): the one FAQ entry that names the product ("VERIDIAN AI")
// inside full sentences (not just a wordmark swap) -- a function of the
// resolved brand label rather than a static const, so a matched host (e.g.
// PROJEXA) reads its own name here instead of the platform default. Every
// other FAQ entry below is genuinely brand-agnostic copy and stays a plain
// const.
function buildDataSecurityFaq(brandLabel: string) {
  return {
    q: `Is my data secure on ${brandLabel}?`,
    a: `Yes. All data is encrypted in transit and at rest by our infrastructure providers. ${brandLabel} is hosted on Vercel and Supabase, both of which hold current SOC 2 Type II certifications, and tenant data is additionally isolated at the database layer via row-level security. Your AI keys (if using BYOK) are encrypted separately and never shared with any third party. ${brandLabel} itself does not currently hold a SOC 2 certification or an independent penetration-test report — if your organisation needs either for a vendor security review, contact our sales team to discuss your requirements.`,
  };
}
...
// GAP-PROJEXA-MARKETING-PAGES-HARDCODED-VERIDIAN (OCID-020 addendum,
// 2026-08-05): `brand` is resolved server-side by the async parent page.tsx
// (real HTTP Host header -> product_branches lookup, same
// resolvePreAuthBrandByHost() contract /login and /signup already use) and
// passed down as a plain prop -- this component does no I/O of its own.
// `null` (no host match, the common/default case) must render byte-
// identical to this page's pre-existing behavior.
export default function PricingPage({ brand }: { brand: PreAuthBrand | null }) {
  const [annual, setAnnual] = useState(true);
  const brandLabel = brand?.brandName ?? "VERIDIAN AI";
  const faqs = [FAQS[0], FAQS[1], buildDataSecurityFaq(brandLabel), FAQS[2], FAQS[3]];
    ...
    <div className="min-h-screen bg-ct-cream">
      ...
      <section className="pt-16 pb-10 text-center px-4">
        ...
        <motion.p
          ...
          custom={2}
        >
          {brand?.tagline ?? "Start free. Scale as you grow."}
        </motion.p>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-ct-cloud2 mt-4 text-sm sm:text-base max-w-lg mx-auto text-balance">
              Join thousands of Indian businesses that trust {brandLabel} to never miss a deadline.
            </p>
