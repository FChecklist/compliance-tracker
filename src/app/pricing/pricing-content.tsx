...
// OCID-020 child UMR-20260805-142629-8087: the "Is my data secure..." FAQ
// answer names the brand inline 3 times -- turned into a function of the
// resolved brand name (defaulting to the original "VERIDIAN AI" literal)
// rather than left as a static string, so it renders correctly under a
// resolved brand's own domain. Every other FAQ is brand-neutral and left
// untouched.
function getFaqs(brandName: string) {
  return [
    {
    ...
    {
      q: `Is my data secure on ${brandName}?`,
      a: `Yes. All data is encrypted in transit and at rest by our infrastructure providers. ${brandName} is hosted on Vercel and Supabase, both of which hold current SOC 2 Type II certifications, and tenant data is additionally isolated at the database layer via row-level security. Your AI keys (if using BYOK) are encrypted separately and never shared with any third party. ${brandName} itself does not currently hold a SOC 2 certification or an independent penetration-test report — if your organisation needs either for a vendor security review, contact our sales team to discuss your requirements.`,
    },
    {
...
// mismatch"): `brand` is resolved server-side by the async parent page.tsx
// (real HTTP Host header -> product_branches lookup) and passed down as a
// plain prop -- this component does no I/O of its own, matching login-form
// .tsx's own established pattern exactly. `null` (the common case: no host
...
// Only the brand NAME/wordmark occurrences are swapped here (nav, FAQ
// copy, CTA banner, footer) -- the actual plan/feature copy (compliance
// items, GSTIN, pricing) is a real product/content decision, not a
// branding-wordmark bug, and is deliberately left untouched, matching
// login/signup's own narrow scope.
export function PricingContent({ brand }: { brand: PreAuthBrand | null }) {
      ...
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-lg shadow-nav">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-heading text-xl text-ct-navy">{brandName}</span>
          </Link>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-ct-cloud2 mt-4 text-sm sm:text-base max-w-lg mx-auto text-balance">
              Join thousands of Indian businesses that trust {brandName} to never miss a deadline.
            </p>
