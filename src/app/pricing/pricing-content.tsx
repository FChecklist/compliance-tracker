...
// BRAND-MISMATCH (same class already fixed on /login, /signup,
// /mfa-challenge -- never previously applied here): this page was a 100%
// "use client" component with no per-route metadata at all, so it inherited
// the root layout's generic "VERIDIAN COGNITIVE AI OS" title, and hardcoded
// "VERIDIAN AI" in the nav wordmark, even though its own CTAs link straight
// into /signup. `brand` is resolved server-side by the async parent
// page.tsx and passed down as a plain prop, mirroring LoginForm/SignupForm's
// own contract exactly -- this component does no I/O of its own. `null`
// (the common case: no host match) renders exactly what this page already
// rendered before this change.
export default function PricingContent({ brand }: { brand: PreAuthBrand | null }) {
      ...
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-lg shadow-nav">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-heading text-xl text-ct-navy">{brand?.brandName ?? "VERIDIAN AI"}</span>
          </Link>
