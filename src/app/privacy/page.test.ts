...
// mismatch"): privacy/page.tsx (via the shared LegalShell) never resolved
// the real pre-auth brand for its nav wordmark. Mirrors terms/page.test.ts
// / signup/page.test.ts's own established convention. Deliberately asserts
// only on the `brand` prop threaded to LegalShell (the nav-wordmark scope
// of this fix) -- NOT on the legal body text, which intentionally stays
...
describe("PrivacyPage (Server Component brand-resolution wiring, nav wordmark only)", () => {
  ...
  test("generateMetadata resolves the real brand into the page title", async () => {
    ...
    expect(metadata.title).toBe("Privacy Policy — PROJEXA")
  })
  ...
  test("an unmatched host keeps the exact original static title", async () => {
    ...
    expect(metadata.title).toBe("Privacy Policy — VERIDIAN AI OS")
  })
})
