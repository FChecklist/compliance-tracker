...
export type { LLMUsage } from "./llm-pricing";
...
// VERIDIAN_Architecture_v2.0 phase_5 (2026-07-26): MODEL_PRICING/
// estimateCostUsd/estimateCacheSavingsUsd moved to ./llm-pricing.ts (a
// dependency-free module, re-exported below for every existing import site
// unchanged) so phase_2's verification-pipeline.ts can compute
// VerificationResult.estimatedCostUsd from phase_5's browser-execution
// engine without pulling this whole ~750-line provider-call module (retry
// logic, 5 providers' HTTP calls) into a client bundle -- this file itself
// is this phase's real Tier-5 (engine-server-escalation) target: browser
// tiers exhaust local capability first (see src/lib/browser-execution/),
// and only escalate to callLLM() below via a real server API route (never
// imported directly from client code, since API keys are resolved
// server-side only) when a request genuinely needs it, per
// registries.credit_spend_governance.
export { MODEL_PRICING, ANTHROPIC_CACHE_READ_DISCOUNT, estimateCostUsd, estimateCacheSavingsUsd } from "./llm-pricing";
