...
//
// AI-Readable API Documentation gap closure (VERIDIAN Review Framework,
// task-20260801-173750, originally August 1 2026): important correction
// to the paragraph above -- "not yet on /api/v1" was stale for a large
// chunk of what it described. By that point src/app/api/v1/projexa/**
// alone had grown to ~200 real, already-shipped route files (CRM leads/
// opportunities, HR/recruitment, payroll, risks, procurement, sales
// invoices/orders, RFIs/submittals, and more) that this generated spec
// simply never documented -- the backend work was done, the OpenAPI
// coverage just never caught up. Added CRM leads/opportunities this pass
// as a first incremental step (the pattern: pick a domain with real
// external-integration demand, add its path+schema entries, repeat).
...
export function generateOpenApiDocument() {
  return {
    ...
    paths: {
      ...
      // ─── AI-Readable API Documentation gap closure ─── First incremental
      // extension since Wave 124's projexa aliasing pass -- both routes
      // below already existed (thin aliases over crm-service.ts, src/app/
      // api/v1/projexa/leads and /opportunities), just weren't documented
      // in this generated spec. No new backend code; inline object schemas
      // below (not $ref'd zod components, since these routes validate
      // ad-hoc rather than via a src/lib/schemas/*.ts file) mirror the
      // exact fields each route's own toLeadShape()/toOpportunityShape()
