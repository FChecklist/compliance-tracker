...
export const ALLOWLIST: Record<string, { count: number; reason: string }> = {
  ...
  "src/app/api/v1/projexa/projects/[id]/approvals/route.ts": { count: 2, reason: "BUILD-001 U-29: the key only scopes the project read (RLS reader context, findReadableProject); the approve POST refuses without a named person (requireActingPerson) and records the write under that person" },
  "src/app/api/v1/projexa/projects/[id]/paste-back/route.ts": { count: 1, reason: "BUILD-001 U-29/U-47: the key only scopes the project read (findReadableProject); the stored proposal and its audit row are attributed to the acting person, the write is refused without one" },
