...
const EXEMPT_ROUTES: Array<{ path: string; category: string; reason: string }> = [
  ...
  {"path":"src/app/api/internal/scheduler-bridge/run/route.ts","category":"INTERNAL_SECRET","reason":"Last hop of the pg_cron -> Edge Function scheduler bridge (BUILD-001 U-40), gated by a constant-time Bearer check against SCHEDULER_BRIDGE_INTERNAL_SECRET that fails closed; runs each due schedule as its owner, a write function only ever becomes a proposal."},
...
describe("authz-gap inventory (R75 Phase 2 drift guard)", () => {
  ...
  test("headline counts match the R75 Phase 2 audit, as of 2026-09-05", () => {
    ...
    // PROTECTED, not exempted and not a known gap: the first three call
    // requireAuthOrApiKey() then requireRoleOrScope(ctx, "manager", ...),
    // the commit route calls requireRole(ctx.dbUser, "manager") -- both are
    ...
    // Sumeet requirement #2 (2026-09-18): +2 mutating route files --
    // v1/projexa/milestones/route.ts's POST and
    // v1/projexa/milestones/[id]/route.ts's PATCH. Both grep-visible
    // PROTECTED (requireRoleOrScope(ctx, "member", "write"), same call as
    // the sibling v1/projexa/schedule/route.ts's POST already in this
    ...
    // already uses.
    //
    // PROJEXA-BUILD-001 U-37 (2026-09-25): +1 mutating route file --
    // v1/projexa/projects/from-document/route.ts's POST (create a project and
    // its BOQ from an uploaded xlsx workbook). Grep-visible PROTECTED, not
    // exempted and not a known gap: it calls requireAuthOrApiKey() then
    // requireRoleOrScope(ctx, "member", "write"), the floor of the sibling
    // project-create route, and refuses a project-scoped key.
    expect(mutating.length).toBe(892) // +1 BUILD-001 U-37 projects/from-document POST, +2 BUILD-001 U-29 v1/projexa/projects/[id]/approvals POST and paste-back POST (both role-gated with requireRoleOrScope), +1 BUILD-001 U-40 scheduler-bridge/run, +35 WO-DPDP-001 dpdp/* mutating routes, +7 WO-DPDP-002/003/004 dpdp/* mutating routes, +2 Sumeet #2 milestones routes, +2 Sumeet #3 billing-claims routes, +1 WO-DPDP-005/007 task-link/[token], +1 AI-link follow-up tasks/[id] PATCH
    expect(protectedCount).toBe(646) // +1 BUILD-001 U-37 projects/from-document POST (requireRoleOrScope, grep-visible), +2 BUILD-001 U-29 approvals POST and paste-back POST (requireRoleOrScope, grep-visible); +12 R75P2P5-G7 (FINAL) real requireRole() gates -- closes the entire authz-gap sweep, 0 KNOWN_OPEN_GAPS remain; +1 R80 BOQ header PATCH; +1 R85A3 P6 cost-visibility config PATCH; +2 R85A3 P7 Excel round-trip diff/apply POSTs; +4 R85 A3v4 Phase 10 boq-scenarios routes; +2 Sumeet #2 milestones routes; +2 Sumeet #3 billing-claims routes; +1 AI-link follow-up tasks/[id] PATCH
    expect(EXEMPT_ROUTES.length).toBe(246) // +1 BUILD-001 U-40 scheduler-bridge/run (INTERNAL_SECRET), +7 R75P2P5-G2 CRM service-layer gates // +2 R75P2P5-G8 training/enrollments ownership-check fixes not visible to the requireRole() grep // +1 R-C17 resend-inbound webhook (Svix-signature-gated, INTERNAL_SECRET) // +35 WO-DPDP-001 dpdp/* routes (29 SERVICE_LAYER_GATED, 4 TOKEN_SCOPED, 2 PUBLIC_BY_DESIGN) // +7 WO-DPDP-002/003/004 dpdp/* routes (all SERVICE_LAYER_GATED) // +1 WO-DPDP-005/007 task-link/[token] (TOKEN_SCOPED)
