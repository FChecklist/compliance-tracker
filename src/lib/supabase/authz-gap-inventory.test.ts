...
describe("authz-gap inventory (R75 Phase 2 drift guard)", () => {
  ...
  test("headline counts match the R75 Phase 2 audit, as of 2026-09-05", () => {
    ...
    // already uses.
    expect(mutating.length).toBe(889) // +1 BUILD-001 U-40 scheduler-bridge/run, +35 WO-DPDP-001 dpdp/* mutating routes, +7 WO-DPDP-002/003/004 dpdp/* mutating routes, +2 Sumeet #2 milestones routes, +2 Sumeet #3 billing-claims routes, +1 WO-DPDP-005/007 task-link/[token], +1 AI-link follow-up tasks/[id] PATCH
