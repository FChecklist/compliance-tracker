...
describe("authz-gap inventory (R75 Phase 2 drift guard)", () => {
  ...
  test("headline counts match the R75 Phase 2 audit, as of 2026-09-05", () => {
    ...
    expect(mutating.length).toBe(889) // +35 WO-DPDP-001 dpdp/* mutating routes, +7 WO-DPDP-002/003/004 dpdp/* mutating routes, +2 Sumeet #2 milestones routes, +2 Sumeet #3 billing-claims routes, +1 WO-DPDP-005/007 task-link/[token], +1 AI-link follow-up tasks/[id] PATCH, +1 BUILD-001 U-37 projects/from-document POST
    expect(protectedCount).toBe(644) // +12 R75P2P5-G7 (FINAL) real requireRole() gates -- closes the entire authz-gap sweep, 0 KNOWN_OPEN_GAPS remain; +1 R80 BOQ header PATCH; +1 R85A3 P6 cost-visibility config PATCH; +2 R85A3 P7 Excel round-trip diff/apply POSTs; +4 R85 A3v4 Phase 10 boq-scenarios routes; +2 Sumeet #2 milestones routes; +2 Sumeet #3 billing-claims routes; +1 AI-link follow-up tasks/[id] PATCH; +1 BUILD-001 U-37 projects/from-document POST
