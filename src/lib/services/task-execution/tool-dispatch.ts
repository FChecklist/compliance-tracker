...
// extracted from task-execution-engine.ts's dispatchTool(), unchanged --
// the structured/deterministic tool-dispatch surface for the free-text
// planning path's global read-only agents plus the small set of real
// write actions whose arguments are never LLM-guessed (see each branch's
// own comment below for why that one is safe). Kept separate from
// engine-dispatch.ts (the VCEL calculation-engine dispatch table) since
// these are a genuinely distinct concern -- data queries/mutations against
// this app's own tables and services, not statutory/mathematical
// calculators. Only public export task-execution-engine.ts still re-exports
// (it's the one function an external caller imports from this file).
