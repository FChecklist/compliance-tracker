...
// closure: task-execution-engine.ts was ~2437 lines; dispatchTool() -- the
// big if/else chain of structured-dispatch tool implementations (global
// read-only agents + a handful of guarded write actions, per each branch's
// own inline comment) -- is one of its two large, mostly-self-contained
// pieces, split out here to shrink the parent file toward something a
// reviewer can actually hold in their head. Re-exported from
// task-execution-engine.ts under its original path/name so its two external
// call sites (src/app/api/v1/projexa/assistant/route.ts,
// src/lib/services/fde-service.ts) and its two internal call sites
// (executeStructuredDispatch(), the free-text planning path) don't change.
