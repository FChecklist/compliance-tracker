...
// VERIDIAN Review Framework gap-closure (2026-08-01, Code Modularity):
// extracted from task-execution-engine.ts's dispatchEngine() + its two
// helpers, unchanged -- this is the VCEL Calculation Engine dispatch
// table (GST/Income Tax/TDS/Accounting/Payroll/Inventory/HR/Banking/
// Procurement/Security/Audit/AI Support/Compliance/Analytics/Logistics/
// Marketing/Project Management/CRM/Sales/Fixed Asset/Data Quality/Document
// Processing engines, ~185 registered engineKeys). By far the largest
// single responsibility in the old monolithic file (over half its lines)
// and the most self-contained -- every case is a plain `await import(...)`
// of a pure calculator plus input coercion, with no dependency on this
// module's own orchestration state. Deliberately a small, explicit
// allowlist switch, not a generic resolver that dynamic-imports whatever
// computation_engines.implementation_ref says -- letting a database row
// control which file gets imported and which export gets called would be
// a real code-execution surface; each case here is a real, reviewed import
// instead. Only called from task-execution-engine.ts's
// executeEngineDispatch() -- not part of this module's own public API.
