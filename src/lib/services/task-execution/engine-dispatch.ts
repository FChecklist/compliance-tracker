...
// closure: task-execution-engine.ts was ~2437 lines; dispatchEngine() --
// the ~1170-line switch/if-chain that maps every VCEL computation_engines
// key to its real, reviewed engine-file import (see this file's own next
// comment for why it's a hard-coded switch, not a dynamic resolver) -- is
// the second of its two large, mostly-self-contained pieces, split out
// here to shrink the parent file toward something a reviewer can actually
// hold in their head. Not imported anywhere outside task-execution-
// engine.ts (called once, from executeEngineDispatch()), so this is a
// plain named export/import, no re-export needed. truthy()/parseNumberList()
// are small helpers used only by dispatchEngine's own cases, moved with it.
