...
describe("what a share link may be minted for (R67 E-12)", () => {
  // R67 merge (2026-09-03): lane D's item D-31 added a third member,
  // attendance_summary, with its own resolver, in the same wave. The rule this
  // list encodes is unchanged -- a type may only appear here if
  // resolveReportShareLink can really render it -- so the vocabulary is the
  // union of both items rather than either one's.
  test("every shipped report type is accepted", () => {
    ...
    expect(assertShareableReportType("attendance_summary")).toBe("attendance_summary")
  ...
  test("a type with no public renderer is refused, and the refusal names the real vocabulary", () => {
    ...
    try {
      ...
      expect((error as ServiceError).message).toBe(
        "Unsupported report type. Shareable reports: work_progress, project_status, attendance_summary"
      )
