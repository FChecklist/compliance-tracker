import { complianceItems, departments, notices, users, tasks } from "@/lib/db";
...
// previously one of two large, unrelated dispatch tables living in that
// single 2,438-line file (the other being engine-dispatch-service.ts's
// dispatchEngine()). Splitting them into their own service files leaves
// task-execution-engine.ts holding only the actual task-execution
// orchestration (LLM planning, guardrails, memory, reflection) -- its own
// re-exported name, per this repo's existing barrel-file convention (see
// src/lib/db.ts re-exporting src/lib/db/schema.ts), so both of this
// function's external call sites (src/app/api/v1/projexa/assistant/route.ts,
// src/lib/services/fde-service.ts) keep importing `dispatchTool` from
// "@/lib/task-execution-engine" unchanged.
//
// Deliberately a small, explicit allowlist of global read-only (plus a
// handful of narrowly-scoped, non-LLM-argument write) agents -- see this
// function's own inline comments for why each write branch is safe to
// auto-dispatch.
