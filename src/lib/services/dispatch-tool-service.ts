...
// task-execution-engine.ts, which re-exports this unchanged so every
// existing `import { dispatchTool } from "@/lib/task-execution-engine"`
// call site keeps working with zero changes. dispatchTool was already a
// fully self-contained responsibility inside that file -- a single
// allowlist switch over `codeReference`, calling only its own imports and
// dynamic service imports, with zero calls into any other
// task-execution-engine.ts-private function -- so moving it here is a pure
// file-boundary change, not a behavior change.
//
// Deliberately read-only-biased: see this function's own inline comments
// below for which branches are safe to auto-dispatch from LLM-generated
// free-text plans (read-only) vs. only from human-click-originated fixed
// inputs (the two explicit write actions).

/**
 * Dispatches one of the platform's global, read-only (plus two explicitly
 * human-click-only write) worker-agent tools by `codeReference`. This is
 * the one real chokepoint every auto-dispatched task-execution-plan step
 * goes through -- see task-execution-engine.ts's executeStructuredDispatch
 * and the free-text planning path for its call sites.
 */
