...
// fixed-asset-specific sibling, built concurrently as its own separate
// report/PR; this file deliberately does not touch fixed-asset accounts or
// erp_fixed_assets/erp_depreciation_schedules. ────────────────────────────
//
...
// version of this report exists to catch. accounting-engine.ts's
// reconcileLedgers() is a generic reference+amount line-matcher -- not used
// here, since this is a single running-total comparison per control
// account, not a two-ledger line-item match.
//
// Honest scope: only AR and AP are included, matching what this codebase
// actually posts to the GL today.
//   - Fixed assets: FI-AA-006's scope (separate report/PR) -- deliberately
//     excluded here.
//   - Inventory/stock (erpAccounts.accountType = 'stock'): confirmed absent
//     by reading erp-inventory-service.ts directly -- recordStockReceipt/
//     recordStockIssue never insert an erp_journal_entries/
//     erp_journal_entry_lines row at all, so there is no GL stock-account
//     balance yet to reconcile the stock ledger against. Adding a "stock"
//     row here would compare a real number to one that doesn't exist --
 ...
 * the GL entry, and the same one the Wave 84 credit-limit gate already uses
 * (never re-fetched, so a later FX-rate change never silently shifts this
 * number out from under an old invoice already on the books).
 *
