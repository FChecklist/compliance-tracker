...
// ServiceError, ExceptionKind and serviceErrorBody live in ./service-error.ts (a leaf, so the ai-work-link-exec bundle does not need this file); re-exported so
// every importer and every `instanceof ServiceError` check is unchanged.
import { ServiceError, serviceErrorBody, type ExceptionKind } from "./service-error"
