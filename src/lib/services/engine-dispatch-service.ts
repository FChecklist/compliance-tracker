...
// Extracted from task-execution-engine.ts (VERIDIAN Review Framework, AI
// Engineering Quality / Code Structure & Modularity gap-closure): this was
// the larger of two unrelated dispatch tables previously living in that
// single 2,438-line file (the other being tool-dispatch-service.ts's
// dispatchTool()) -- roughly 1,175 lines on its own, covering every VCEL
// computation-engine category (GST, Math, Costing, Income Tax, TDS/TCS,
// Accounting, Payroll, Inventory, HR, Banking, Procurement, Security,
// Audit, AI Support, Compliance, Analytics, Logistics, Marketing, Project
// Management, CRM, Sales, Fixed Assets, Data Quality, Document
// Processing). Zero coupling to task-execution-engine.ts's LLM-planning,
// guardrail, and reflection orchestration beyond the one DB-backed case
// (gst_return_validation_engine) below, so this is a natural, low-risk
// service boundary. Exported (unlike its pre-extraction, module-private
// form) so the service module has a real public surface, but still
// deliberately called from exactly one place today
// (executeEngineDispatch() in task-execution-engine.ts) -- it stays a
// single reviewed allowlist switch (see its own comment below), not a
// generic resolver.
