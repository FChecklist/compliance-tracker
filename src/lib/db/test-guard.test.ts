...
// test -- no real DB connection is opened here, so this needs no
// DATABASE_URL and is safe under bun test --isolate.
...
describe("assertTestDatabase (P2.3 destructive-test guard)", () => {
  test("does not throw for this repo's known dev/test project", () => {
    expect(() =>
      assertTestDatabase("postgresql://postgres.pcrjmlpuqsbocqfwoxod:pw@aws-1-ap-south-1.pooler.supabase.com:6543/postgres")
    ).not.toThrow()
  })
  ...
  test("does not throw when unset (nothing to connect to, nothing to guard)", () => {
