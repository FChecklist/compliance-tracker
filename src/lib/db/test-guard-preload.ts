...
// itself (e.g. in a test). Checks both connection-string env vars this repo
// uses -- see db/index.ts (DATABASE_URL) and tenant-scoped.ts
// (APP_RUNTIME_DATABASE_URL).
import { assertTestDatabase } from "./test-guard"

assertTestDatabase(process.env.DATABASE_URL)
assertTestDatabase(process.env.APP_RUNTIME_DATABASE_URL)
