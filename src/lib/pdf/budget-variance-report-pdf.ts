...
export type BudgetVarianceReportPdfData = {
  ...
  totals: { budget: number | null; vendorAmount: number; variance: number }
...
export function generateBudgetVarianceReportPdf(data: BudgetVarianceReportPdfData): ArrayBuffer {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          ...
          "", money(data.totals.vendorAmount), money(data.totals.variance),
        ],
      ],
  ...
  drawFooterNote(
    doc,
    subTaskCount > 0
      ? `${subTaskCount} weighted sub-task line${subTaskCount === 1 ? "" : "s"} are included in their parent line's figures. Positive variance is over budget. Generated by VERIDIAN AI.`
      : "Positive variance is over budget. Generated by VERIDIAN AI."
  )
