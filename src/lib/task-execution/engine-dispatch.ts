 ...
 * Code Quality, 2026-08-15): extracted from task-execution-engine.ts (see
 * that file's own header, and tool-dispatch.ts's, for the full split
 * rationale). This module owns `dispatchEngine()` alone -- the deliberately
 * small, explicit allowlist switch (NOT a generic resolver that dynamic-
 * imports whatever computation_engines.implementation_ref says; letting a
 * database row control which file gets imported and which export gets
 * called would be a real code-execution surface) that resolves an
 * engineKey to one of the ~185 registered VCEL computation engines' real,
 * reviewed pure functions. GST Engine (16/16), Mathematical Computation
 * Engine (10/13), Costing Engine (8/8), and the rest of the categories
