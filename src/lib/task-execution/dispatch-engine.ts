// Computation-engine dispatcher, extracted from task-execution-engine.ts
// (AI Engineering Quality gap-closure, 2026-08-15: see dispatch-tools.ts's
// header, same directory, for the full split rationale). Pure extraction,
// no behavior change -- every case below is byte-identical to the original
// dispatchEngine()/truthy()/parseNumberList(), which were not imported by
// anything outside task-execution-engine.ts (grep-verified before this
// split), so this module has no external re-export obligation the way
// dispatch-tools.ts's dispatchTool() does -- task-execution-engine.ts is
// still the only caller, now via an import instead of same-file.
...
// Exported (not just module-private) so dispatch-engine.test.ts can cover
// these two pure decision points directly -- matches this codebase's
// established pure-function-only test convention (see
// task-execution-engine.test.ts's own header for the precedent this
// follows) rather than only exercising them indirectly through the
// DB/LLM-touching dispatchEngine() switch, which stays untested here.
