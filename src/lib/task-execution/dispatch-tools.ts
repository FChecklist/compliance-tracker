...
// Pure extraction, no behavior change -- every branch below is
// byte-identical to the original dispatchTool(), still re-exported from
// "@/lib/task-execution-engine" so both existing external call sites
// (src/app/api/v1/projexa/assistant/route.ts,
// src/lib/services/fde-service.ts) keep importing from the same path.
//
// Handles the handful of global read-only agents free-text task planning is
// allowed to auto-dispatch (see task-execution-engine.ts's own header for
// why read-only-only), plus a few real write actions whose arguments are
// never LLM-generated (update_compliance_status, create_compliance_item --
// see each branch's own comment for why those are safe here).
