...
// Structure & Modularity, 2026-08-15): dispatchTool and the chain-
// completion helpers used to be defined inline in this file -- both were
// already fully self-contained responsibilities (dispatchTool calls no
// other task-execution-engine.ts-private function; the chain-completion
// pair is only ever called from updateTaskStatusAndReflect below), so they
// now live in their own service files and are re-exported/imported here
// unchanged. dispatchTool is re-exported (not just imported) so every
// existing `import { dispatchTool } from "@/lib/task-execution-engine"`
// call site elsewhere in the codebase keeps working with zero changes.
export { dispatchTool } from "@/lib/services/dispatch-tool-service";
import { recordChainWorkerAgentEdges, enforceChainMonitoringRules } from "@/lib/services/chain-completion-service";
...
// recordChainWorkerAgentEdges and enforceChainMonitoringRules now live in
// chain-completion-service.ts (imported above) -- see that file's own
// header for the full history/reasoning; both are called unchanged from
// updateTaskStatusAndReflect above.
