...
// See tool-dispatch.ts's own header for why dispatchTool() lives there --
// imported (not just re-exported) since this file's own dispatch paths
// (executeStructuredDispatch, the free-text planning path below) still
// call it directly, and re-exported under the same name/path so its two
// external call sites (src/app/api/v1/projexa/assistant/route.ts,
// src/lib/services/fde-service.ts) are unaffected.
