...
// F089) optional `role` param to this same signature after this PR's
// branch point, used below to redact financial fields from construction
// dashboard/budget results for sub-manager roles. Preserved here rather
// than dropped by the extraction -- see the CONSTRUCTION_TOOL_CODES branch
// and construction-tools.ts's dispatchConstructionTool() for where the
// redaction now lives (threaded through as a param instead of computed
// inline, since that logic moved out of this file).
export async function dispatchTool(db: TenantDb, orgId: string, userId: string, codeReference: string, context?: { taskId?: string; inputs?: Record<string, unknown> }, role?: string | null): Promise<unknown> {
  ...
  // get_penalty_estimate (compliance) and the 6 GST Reconciliation codes
  // are now routed via COMPLIANCE_TOOL_CODES/GST_TOOL_CODES below -- see
  // compliance-tools.ts / gst-tools.ts for the relocated implementations
  // (unchanged in behavior, just moved).
  ...
  // here. (Landed on main after this PR's branch point -- kept inline as-is,
  // not part of this refactor's compliance/gst/construction split.)
  ...
  // Construction Intelligence (PROJEXA) is routed via CONSTRUCTION_TOOL_CODES
  // below. R48 gap-closure (2026-08-30, F089/F059): `role` is threaded
  // through to dispatchConstructionTool() so it can apply the same
  // manager-or-higher financial-field redaction it did when this logic was
  // still inline here -- see construction-tools.ts for where that check now
  // lives. `role` undefined (caller not yet wired to pass it) is treated as
  // "unknown, don't redact" there too, preserving prior behavior.
