...
// Code Structure & Modularity gap-closure (VERIDIAN Review Framework):
// dispatchTool()/dispatchEngine() used to be defined directly in this file
// (~1,435 of its lines combined) -- extracted into their own service
// modules (see each file's own header for why) and re-exported below so
// every existing external call site keeps importing them from
// "@/lib/task-execution-engine" unchanged, matching this codebase's own
// barrel-file convention (src/lib/db.ts re-exporting src/lib/db/schema.ts).
export { dispatchTool } from "@/lib/services/tool-dispatch-service";
import { dispatchEngine } from "@/lib/services/engine-dispatch-service";
