...
// layer (executeTask() and everything it directly calls) and imports
// dispatchTool/dispatchEngine like any other dependency. dispatchTool is
// re-exported below unchanged since two other files import it directly
// from "@/lib/task-execution-engine" (src/app/api/v1/projexa/assistant/
// route.ts, src/lib/services/fde-service.ts) -- moving it without a
// re-export would have broken both without a compensating change to files
// outside this gap's declared scope.
import { dispatchTool } from "./task-execution/tool-dispatch";
import { dispatchEngine } from "./task-execution/engine-dispatch";
export { dispatchTool };
