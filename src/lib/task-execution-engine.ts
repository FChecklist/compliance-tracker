...
// AI Engineering Quality gap-closure, 2026-08-15: dispatchTool()/
// dispatchEngine() used to live inline in this file (they made it a
// 2,437-line monolith combining three distinct responsibilities). Both are
// now their own modules under ./task-execution/ -- see each one's own
// header for the split rationale. dispatchTool is re-exported here (not
// just imported) because two files outside this module still import it as
// "@/lib/task-execution-engine" (src/app/api/v1/projexa/assistant/route.ts,
// src/lib/services/fde-service.ts) -- re-exporting keeps that import path
// working unchanged rather than forcing every external caller to move to
// the new location too.
import { dispatchTool } from "./task-execution/dispatch-tools";
import { dispatchEngine } from "./task-execution/dispatch-engine";
export { dispatchTool };
 ...
 * (see dispatch-tools.ts's dispatchTool()) -- executes them for real against
 * the org's real data and records the output. Posts a one-message summary to
