...
**Update (same day):** with explicit user confirmation, this is now resolved rather than just flagged. `veridian-compliance-ai` is confirmed as the one true app; `pcrjmlpuqsbocqfwoxod` is confirmed as the one true database. All five Supabase-related env vars on `veridian-compliance-ai` were repointed to `pcrjmlpuqsbocqfwoxod` (DB password was reset in the process since the old one couldn't be verified — see Change Log #5/#6). A fresh deployment was triggered to pick up the corrected `DATABASE_URL`. **Still pending:** live verification that login/dashboard work correctly against the new target post-deploy, then — only after that passes — dropping the old `compliance` schema from `jusqumifsmtcaujqyjuy` (never touching that project's other schemas, which belong to MeetTrack) and deleting the dead `compliance-tracker` Vercel project.
