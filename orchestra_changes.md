...
Pattern established by the 4 done so far: import `withTenantContext` from `@/lib/db/tenant-scoped` instead of `db` from `@/lib/db`; wrap the query body in `withTenantContext({ orgId }, (db) => ...)`; return early if `orgId` is falsy; while migrating, actually read the route and check whether it has the same class of bug as the three found so far (query ignores the real user/org and falls back to "first match in the whole database"). Each one should get its own `tsc --noEmit` + `eslint` + full `npm run build` pass before committing, same as the four done so far — don't batch many at once without checking each.

- [x] `departments` (GET, POST)
- [x] `notifications` (GET), `notifications/[id]/read` (PATCH)
- [x] `users` (GET, POST)
- [ ] `compliance` (GET, POST), `compliance/[id]` (GET, PATCH/DELETE), `compliance/[id]/comments`, `compliance/import`, `compliance/overdue`, `compliance/recur`, `compliance/stats` — the largest and highest-traffic group, do these carefully
- [ ] `notices` (GET, POST), `notices/[id]`, `notices/stats`
- [ ] `challans/[id]`
- [ ] `audit` (GET), `audit-points/[id]`
- [ ] `departments/[id]`
- [ ] `documents/extract`
- [ ] `ingest`, `ingest/[batchId]`, `ingest/[batchId]/confirm`, `ingest/[batchId]/items/[itemId]`
- [ ] `mcp/tokens` (the `/api/mcp` route itself deliberately stays on the service-role admin client — see analysis.md — but `mcp/tokens`, which issues those tokens, should be tenant-scoped)
- [ ] `me`
- [ ] `settings/api-keys`, `settings/api-keys/[id]`, `settings/webhooks`, `settings/webhooks/[id]` — note: `api_keys` and `webhooks` tables don't exist in the live database at all (per analysis.md); these routes may currently be non-functional regardless of RLS, worth checking what they actually do before migrating
- [ ] `search/semantic` (uses `lib/embeddings.ts`'s own raw client — needs its own tenant-scoping approach, not a drop-in `withTenantContext` swap)
- [ ] `ai/orchestrate` (already has its own `requireAuth()` per prior fix; check whether its DB reads need tenant scoping too)

Once every route above is migrated and verified, the remaining step to actually finish Wave 1 is switching `DATABASE_URL` itself away from `postgres` (or removing the `postgres`/`service_role` bypass policies) so there's no leftover RLS-bypassing path left for anything to accidentally use.
