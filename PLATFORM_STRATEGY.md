...
### 29.4 Phase 0 status -- CORRECTED 2026-07-14 (was stale)

**Corrected 2026-07-14** (found stale during a Priority 14 sub-agent's Phase 0 re-verification pass -- this section was never updated after Phase 0 actually shipped the next day): Phase 0 is DONE, not "not yet started." WAVE-170/PR #251 (merged 2026-07-13, audited PASS same day by an independent claude-code session, see `ai-os/boss/COMPLETED.yaml`) built exactly what the Phase 0 bullet above describes: `MonitorReportFields` + `validateMonitorReportFields()` (`src/lib/monitor-protocol.ts`), the `monitor_agents`/`monitor_task_state` registry (`drizzle/0173_narrow_monitor_agents_phase0.sql`, applied live), `escalation-ladder.ts` extended with a single-owner claim lock and a persisted retry/timeout counter (`evaluateEscalationClaim()`/`claimEscalation()`), and the one Tier-1 rule-engine monitor wired end-to-end on `APPROVAL_GRANTED`/`APPROVAL_REJECTED` (`src/lib/monitors/approval-decision-monitor.ts`, called from `src/app/api/approvals/[id]/route.ts`). Independently re-verified 2026-07-14 (not just trusting the prior PASS): `tsc --noEmit` clean, `eslint .` 0 errors, full test suite 1113/1113 passing, including `escalation-ladder.test.ts`'s dedicated single-owner-lock/retry/timeout coverage and `monitor-protocol.test.ts`'s field-validation coverage. Phase 1 (expand event coverage beyond these 2 proven events to the other ~28) and Phase 2 (general Tier 2/3 executors for that ~30-event list) remain genuinely open -- a Tier-3 executor exists for a related but DIFFERENT purpose (dispatch-completion monitoring, see section 31.4 / `src/lib/monitors/dispatch-completion-monitor.ts`), which is not the same thing as Phase 2 of this section's own ~30-event list. See `ai-os/MASTER-TRACKER.yaml`'s `GAP-NARROW-MONITOR-ESCALATION` entry for the corrected open-scope description.
...
| Agent Performance (APR) | **Cost gap closed 2026-08-15** (AI Model Lifecycle & Benchmarking: Ongoing Quality Monitoring). `model-scorecard-service.ts` now merges in real spend from `token-usage-service.ts`'s own ledger (`token_usage_ledger`, grouped by model) via `attachModelCost()` -- honestly model-granular, not tier-split (that ledger has no complexity-tier column), documented via `COST_GRANULARITY_NOTE` rather than fabricating a per-tier split. Iteration count and hallucination-score remain genuinely NOT computable from any persisted data today (no infrastructure to record either), documented via `ITERATION_COUNT_NOTE`/`HALLUCINATION_SCORE_NOTE` rather than invented. See the Agent Review (ARR) row below -- this same pass also built the MODEL-granularity counterpart of ARR (`model-lifecycle-service.ts`), since ARR's own promote/maintain/retrain/deprecate cycle runs at role_key granularity and a role going stale doesn't tell you whether the underlying model itself should keep receiving judgment-tier work platform-wide. |
...
### 30.4 Status (updated 2026-08-15)
...
**Added 2026-08-15 (AI Model Lifecycle & Benchmarking: Ongoing Quality Monitoring):** Agent Performance's own two documented sub-gaps -- no cost field, no MODEL-granularity lifecycle cycle (ARR only ever covered role_key granularity) -- are now closed. `model-scorecard-service.ts` merges in real per-model spend from `token_usage_ledger`. `model-lifecycle-service.ts` (`drizzle/0225_model_lifecycle_reviews.sql`, `model_lifecycle_reviews` -- append-only, mirrors `agent_review_records`' own shape at model instead of role_key grain) runs the identical deterministic promote/maintain/retrain/deprecate verdict `agent-review-service.ts`'s `computeReviewVerdict()` already provides -- reused directly rather than re-implemented, confirmed by direct read to be granularity-agnostic (count-based, no role_key dependency). New veridian_admin-gated GET+POST `/api/ai/team/model-lifecycle` route, matching `/review-registry`'s own API-only convention. Hallucination-score remains honestly NOT computable (no persisted per-dispatch signal exists anywhere in this codebase today) -- documented via `HALLUCINATION_SCORE_NOTE`, not fabricated.
...
Boss directive 2026-07-13 (second pass, after the 5-question metadata investigation in §§ closed by PR #250): 8 named pain points affecting every agent working on this codebase (Claude Code, z.ai GLM 5.2, or any future coding agent) -- drift, scattered work-history, undocumented decisions, no pre-work SOP, hallucination/infinite loops, unclear "what VERIDIAN AI OS even is," no live completion percentage, no unified open/closed/audited view -- plus a specific proposal: use GPT-OSS-120B, narrowly instructed, purely for cross-agent monitoring/reporting/escalation (never coding or reasoning).

### 31.1 Each pain point checked against what's actually in the codebase today, not assumed

| # | Pain point | Real current coverage | Verdict |
|---|---|---|---|
| 1 | Drift | `CLAUDE.md` is auto-injected into every interactive Claude Code session by the CLI itself (unconditional, no gap). `ai-workforce-agent.mjs`'s `fetchGovernancePreamble()` (PR #250) now does the same for every z.ai/OpenRouter headless dispatch -- previously silently missing, now closed. **Residual**: nothing forces a long-running interactive session (like this one) to re-read `ai-os/OS.yaml` mid-session as new work gets merged; that's a discipline, not code, addressed in §31.4. |
| 2 | One place for all work/instructions/other-agents'-work | `ai-os/OS.yaml` (PR #250) now indexes every real governance document with a one-line "what it's for," enforced non-negotiable by `check-metadata-index-coverage.mjs` in CI (same registered/exempted, nothing-in-neither pattern as the asset registry). **Real remaining gap found while writing this section**: `ai-os/boss/COMPLETED.yaml` -- the doer+auditor completion log OS.yaml itself points to -- was last updated at WAVE-165 and is missing every task from this session (PR #246-253, 8 real merged PRs). This is drift, demonstrated live, not hypothetical. Backfilled in §31.4. |
| 3 | Linking of all documents (done/pending/decided/why-not/to-do) | `ai-os/MASTER-TRACKER.yaml` (open, with real owner/blocker per item) + `ai-os/boss/COMPLETED.yaml` (closed, doer+auditor) + `PLATFORM_STRATEGY.md` (decided/why, one numbered section per evaluated initiative, evaluated→design→deferred convention) together cover "done/pending/decided/why-not" as *separate, cross-referenced* files by design (MASTER-TRACKER.yaml's own header explains why it deliberately does NOT fold COMPLETED.yaml or BOARD.yaml into itself -- different purposes, folding would destroy information). `ai-os/OS.yaml` is the index tying them together. **Gap**: the linking is real but manual -- reading 3 files to answer "what's the status of X" still works, but nothing computes a single merged view. Closed in §31.4 (unified status view). |
| 4 | SOP before any work starts | `task-tightening.ts`'s `TightTask`/`assembleTightTaskPrompt()` (precise-instruction contract) + `fetchGovernancePreamble()` (governance files prepended) together are the SOP-injection mechanism for dispatched agents. For *this* session specifically: none of it is optional -- `CLAUDE.md` loads before I see the first message. **No further gap for interactive sessions**; the residual is the same as #1's. |
| 5 | Hallucination / unending loops | `escalation-ladder.ts`'s `MAX_RETRY`/`TIMEOUT`/single-owner-lock (PR #251) + `model-tier-eligibility.ts`'s tier restrictions (a model proven to burn its iteration budget on integrative work, e.g. GPT-OSS-120B, is mechanically blocked from being routed there) are the two real anti-loop mechanisms today. **Gap**: both are Phase-0/proof-of-concept scale (wired to `APPROVAL_GRANTED`/`APPROVAL_REJECTED` only) -- not yet watching the general case of "an AI Dev Team dispatch that never reports completion." `listStuckActivities()` (PR #250) is the detection half of that (>24h in a non-terminal stage); nothing yet *acts* on what it detects. Closed in §31.4 (Monitor Phase 1+2). |
| 6 | Clear understanding of what VERIDIAN AI OS is / how it works | Fragmented across `ai-os/OS.yaml` (index), `ai-os/system-tree/` (Tree 3 -- what's actually built, per-domain), `CLAUDE.md` (stack/structure). No single "start here, plain-language" answer to "what is this thing and how does it actually work end-to-end" exists -- `ai-os/OS.yaml`'s own header admits the closest thing to a "Brain" is itself, an index, "not a working cognitive architecture." **Real gap.** Closed in §31.4. |
| 7 | % completed / % pending | `ai-os/STATUS-REPORT.md` exists but is explicitly **point-in-time and manually reconciled** -- its own header says "Originally built 2026-07-12," last updated same day, and its own methodology note describes a multi-hour manual domain-by-domain reconciliation pass to produce the numbers. As of today (2026-07-13) it is already one day stale and does not reflect PR #246-253. **Real gap**: there is no live/computed version. Closed in §31.4. |
| 8 | Open / closed / reviewed-by / audited-by | Individually real: `MASTER-TRACKER.yaml` (open, with owner), `COMPLETED.yaml` (closed, doer+auditor by name), `audit-protocol.ts`/`mandatory-audit-check.yml` (every PR's audit verdict, structured, CI-enforced since PR #248). **Gap**: three separate files, no single queryable answer to "show me everything, open and closed, with who touched it." Closed in §31.4 (same unified view as #3/#7). |

**Something not in the original 8, worth naming**: decision *history* -- when a design choice is later revisited, is the earlier reasoning still visible, or silently overwritten? Checked: `PLATFORM_STRATEGY.md` sections are append-only by convention (never edited to erase a prior "explicitly deferred"), and `COMPLETED.yaml` is explicitly documented as append-only, never edited after the fact. Real, already-followed discipline -- named here so it's visible rather than assumed.

### 31.2 The GPT-OSS-120B-as-monitor proposal, evaluated against what's already designed

This is not a new idea competing with existing infrastructure -- it is **§29.3's own Phase 2**, already scoped in detail before today and never built: *"wire the Tier 2 (Llama 3.1 8B/Cerebras) and Tier 3 (GPT-OSS-120B + glm-5.2 fallback) executors for the narrow remainder of monitors that genuinely need language understanding."* Three independent facts converge on why GPT-OSS-120B specifically fits a monitor/report/escalate role and nothing broader:

1. `model-tier-eligibility.ts` already restricts GPT-OSS-120B to **mechanical tier only** -- proven twice in this session's own history to burn its full iteration budget on integrative-shaped work. Monitor/report/escalate is *definitionally* mechanical: read a status, compare against a rule or a short rubric, emit one of a handful of structured fields. It is exactly the task shape GPT-OSS-120B has a confirmed win on (the 6-tool infra scaffolding wave), and never asked to design, refactor, or make a judgment call.
2. AGENTS.md Operating Rule 8 (90-day quality mandate, in effect through ~2026-10-08) explicitly carves out this exact case: *"the platform-default floor tier... exists precisely so cheap/fast capacity is used for genuinely low-stakes work; the mandate is about not letting cost be the reason something under-trained or under-verified ships."* A monitor's report is not itself the work being verified -- it is what triggers escalation to a human or a judgment-tier model when something looks wrong. Using GPT-OSS-120B here does not weaken quality; routing *coding or design* work to it would.
3. `MonitorReportFields` (PR #251) was already built with a Tier 2/3 executor in mind -- its own header states a model-computed report produces "this same MonitorReportFields shape, just computed by a model call instead of a pure comparison." No redesign needed, only a new executor function that calls GPT-OSS-120B with a narrow rubric and validates the output through the same `validateMonitorReportFields()` gate a rule-engine monitor already goes through.

**What this is not**: GPT-OSS-120B does not gain any new authority. It cannot approve/reject/merge/edit code. Its only output is a `MonitorReportFields` record (`status: ok|escalate`), consumed by the same deterministic `escalation-ladder.ts` machinery a Tier-1 rule-engine monitor already uses. If GPT-OSS-120B's own report is malformed or ambiguous, `validateMonitorReportFields()` rejects it the same way it would reject a human's sloppy report -- the model is not trusted more than the mechanism trusts anyone else.

### 31.3 What NOT to build (avoiding duplication, per the Boss's explicit instruction)

- Not a new tracker file -- `MASTER-TRACKER.yaml`/`COMPLETED.yaml` stay as the two source files; §31.4's "unified view" *reads* them, never replaces or forks them.
- Not a new escalation mechanism -- `escalation-ladder.ts` (CSEO→COO→Super Boss, MAX_RETRY/TIMEOUT/single-owner-lock) is reused as-is.
- Not a new report schema -- `MonitorReportFields` is reused as-is; only a new executor (model-backed instead of rule-engine-backed) is added.
- Not a second governance-preamble mechanism -- `fetchGovernancePreamble()` (PR #250) is the one injection point; a monitor dispatch uses the same function, not a copy.
- Not a rewrite of `STATUS-REPORT.md`'s methodology -- its domain-level Tree1-vs-Tree3 reconciliation approach is sound and stays as the periodic *deep* audit; §31.4 adds a *shallow, always-current* counter (open-item count vs. closed-item count) alongside it, not instead of it.

### 31.4 Phased implementation plan

- **Phase A (this pass)**: (1) Backfill `COMPLETED.yaml` with PR #246-253's missing entries -- closes the drift instance found in 31.1 row 2 directly, doer+auditor per AGENTS.md Rule 7(d). (2) `ai-os/BRAIN.md` -- single plain-language "what VERIDIAN AI OS is and how it works end-to-end," built from real inspection of `ai-os/system-tree/` and `src/`, not invented, cross-linked from `OS.yaml`'s `brain_pointer` section. Closes pain point 6. (3) A `scripts/compute-status.mjs` + `/api/ai/team/governance-health` extension: parses `MASTER-TRACKER.yaml`'s `open_items` count and `COMPLETED.yaml`'s `entries` count into a live percentage, plus a merged open/closed/owner/auditor listing -- shallow and always-current, explicitly not a replacement for `STATUS-REPORT.md`'s deeper periodic reconciliation. Closes pain points 3, 7, 8.
- **Phase B (this pass)**: Generalize the Narrow Monitor beyond `APPROVAL_GRANTED`/`APPROVAL_REJECTED` (§29 Phase 1) to watch AI Dev Team dispatch completion generally, and add the Tier 3 GPT-OSS-120B executor (§29 Phase 2) that consumes `listStuckActivities()`'s output, evaluates it against a narrow rubric ("did this dispatch produce the handover fields it was tightened to produce, yes or no"), and escalates via the existing `escalation-ladder.ts` on `status: escalate`. Closes pain point 5 for the general case, and is the concrete answer to the GPT-OSS-120B-as-monitor proposal.
- **Explicitly deferred**: Tier 2 (Llama 3.1 8B/Cerebras) stays deferred until Tier 3's GPT-OSS-120B path is proven in production -- §29's own sequencing, not revisited here without a reason to.
