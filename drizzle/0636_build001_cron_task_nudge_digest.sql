...
--
-- SOURCE
--   supabase/prepared/cost001/10_task_nudge_digest.sql: the function text is copied unchanged except that the dollar-quote tag is fn where the prepared file used the bare pair. That file holds the
...
--  Notification dedup stays ON (decisions 3 and 7). The digest is kept.
--  The digest's dedup key (kind task_nudge_digest) differs from cron_task_overdue's (kind task_overdue, migration 0633), so the two
--  do not suppress each other: an assignee with an overdue task can receive one row from each. Migration 0633 runs the overdue
--  check inside its wrapper.
--
