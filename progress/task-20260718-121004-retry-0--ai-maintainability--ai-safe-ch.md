...
- [x] Wired both into `.github/workflows/ci.yml` as new jobs (`new-test-coverage`, `test-coverage-gap-report`); the new-test-coverage job needs `fetch-depth: 0` on checkout since the default shallow checkout has no `origin/main` history to compute a merge-base against.
...
- [x] Verified both scripts manually against real repo state (success path + a deliberately-introduced violation, reverted) before wiring to CI.
...
- [ ] Add unit tests for the two new scripts (`report-test-coverage-gap.test.ts`, `check-new-test-coverage.test.ts`) exercising the pure exported functions — in progress.
- [ ] Run full `bun test`, `bunx tsc --noEmit`, `bun run lint` locally before opening the PR.
- [ ] Register claim in `ai-os/boss/ACTIVE-CLAIMS.yaml`, commit, push, open PR.
