...
**REAL OWNER ARCHITECTURE DECISION, 2026-08-06 (`UMR-20260806-025638-cbea`)**: given verbatim — "VERIDIAN is one single application, PROJEXA-AI.COM is a thin client brand extension only, not a separate product, more industry brands are planned the same way, product is VERIDIAN sold through different brand names." This is the standing architectural rationale for the 2026-08-02T13:50 UTC Wave 10 redo above, not a new cutover — **independently re-verified live on 2026-08-06T03:09 UTC, before recording this, that no cutover action was needed**: `curl -I https://projexa-ai.com/` and `https://www.projexa-ai.com/` both resolve through Vercel's shared anycast IPs (`216.198.79.1`/`.65`) with `x-vercel-id` region `sin1`, matching `compliance-tracker`'s own `vercel.json` (`regions: ["sin1"]`), not the standalone `projexa` project's `fra1`/`iad1` region seen in the pre-Wave-10-redo state. The binding logged above (`WAVE-10-REDO`, PR #720) is still the live, current state; `vercel domains add ... --force` was **not** re-run since there was nothing to change. This decision also formally supersedes the still-open, unattributed `WAVE-201` entry from unmerged branch `ca9e6432` (never merged, no PR, `auditor.verdict: pending`) — that entry recorded a same-direction re-cutover with no verifiable Owner attribution; this dated, cited Owner decision is now the attributed record for this architecture going forward.

**Production ready (updated)**: domain-architecture question resolved by real Owner decision and executed, and now has a standing architectural rationale on record; still No overall — Hobby tier and no Rule 7(e) sign-off remain real, open gaps.
