...
## Amendment (2026-08-02T16:33Z) — PR-to-UMR mapping for 6 real merged PRs, real-vs-governance classification

**Status: filed under this same parent UMR — `UMR-20260802-104058-25ba` — no new UMR, no new audit, no new canonical artifact.** Owner directive OCID-20260802-013. This amendment inherits the parent's status (item-by-item completeness above is unchanged by this section; this section only adds traceability + honesty classification for 6 PRs that merged tonight, 4 of them already implicated in existing rows above — items 10, 12, 13, 14).

**Method**: every UMR id below was looked up directly against the real backing store (`/opt/veridian/ai-os/memory/superboss-register.sqlite`, `umr_tasks` table) — first via `resource_governor.py --query-umr --search`, then, after that CLI's FTS5 index proved to only cover `task_identity`/`source_trigger`/`logs_ref` (not `umr_id` itself, confirmed by 0-result searches on exact UMR-id strings that do exist in the table), via a direct `sqlite3`/`python3` query on `umr_id`. Every commit hash, merge timestamp, CI conclusion, and audit-comment verdict below was pulled live via `gh pr view`/`gh api` against the real GitHub state at write time, not recalled from any prior write-up.

### PR → UMR mapping table

| PR | Repo | Real dispatch UMR | `task_identity` / `unit_name` | UMR status (live DB) | Real vs governance |
|---|---|---|---|---|---|
| #716 | compliance-tracker | `UMR-20260802-111942-8d94` | `owner-task-20260802-111941-3158536` | `running` (never flipped to `completed` in the DB despite the PR merging — see caveat below) | **Governance/documentation.** Writes the already-compiled matrix to a durable file; adds 2 registry entries. |
| #717 | compliance-tracker | `UMR-20260802-113654-271b` | `owner-task-20260802-113652-3211579` | `running` | **Governance/documentation, explicitly "plan only — nothing implemented"** (the PR's own words). Registers a KERNEL_AMENDMENT instruction + a future implementation plan; no code. |
| #692 | compliance-tracker | `UMR-20260802-040056-5319` (**not** the master directive `UMR-20260802-034545-3388` cited in the PR body — that one is `status=rejected_duplicate`/cancelled; `040056-5319` is the real per-task dispatch, `unit_name: veridian-worker@task-20260802-040131-parallel-job--collate-existing-module-en.service`, exact match to the PR's own branch) | `owner-task-20260802-040054-1672871` | `completed` (closed 2026-08-02T10:40:00Z per PM decision `UMR-20260802-103748-11da`) | **Governance/documentation.** Collates and cross-references existing infrastructure into one reference doc; PR body states "no code or logic touched." |
| #720 | compliance-tracker | `UMR-20260802-134939-145d` | `owner-task-20260802-134938-3643943` | `running` | **Governance/documentation for the PR's own diff** (45-line additive log entry in `COMPLETED.yaml`) — but logs a **real infrastructure action**: the cited UMR's own execution was 2 live Vercel API calls (`DELETE`/`POST` on domain bindings), independently re-verified via `curl`/DNS/page-body fetch (see item 12 above). The PR itself is paperwork; the thing it documents was real. |
| #14 | veridian-scripts | `UMR-20260802-090702-c813` (covers only the PM-triage-escalation extension; the base stuck-task/heartbeat commit on the same PR cites no UMR, only "PM directive" in the PR body) | `owner-task-20260802-090700-2718320` | `running` | **REAL PRODUCT IMPLEMENTATION — the only one of the 6.** Adds `find_stuck_tasks()`, `write_stuck_tasks_heartbeat()`, `should_triage_pm()`, `_invoke_triage_claude()`, `append_pm_triage_alert()`, `_summarize_evidence()` to `dispatch-tick.py`, plus 2 new test files (`test_stuck_task_heartbeat.py` 27 assertions, `test_pm_triage.py` 18 tests via pytest), all real and passing per the PR's own test-plan log; PR body also documents a real dry-run against the live box's 842 `task.yaml` files (424 genuinely stuck) and a real end-to-end `claude -p` invocation within a $0.50 budget cap. |
| #122 | claude-control | `UMR-20260802-154546-ceb6` | `owner-task-20260802-154546-4016093` | `rejected_duplicate` (correctly — dispatch-gateway's own duplicate-guard found PR `claude-control#122` already existed for this exact `task_identity` and skipped a redundant redispatch; this is the guard working as designed, not an error) | **Governance/registry only.** Adds 9 pre-existing-feature entries to `MASTER_INDEX.yaml`, explicitly "0 modifications to any existing content" per the PR's own summary — catalogues code that already existed, does not write any. |

### Honest discrepancy found, not silently resolved

PR #717's own body cites `UMR-20260802-121158-d557` as its reason for existing as a split-out PR from #697. That UMR **does exist live** in `umr_tasks` (`status=rejected_duplicate`), but its `reason` field says the duplicate-guard matched it to **`compliance-tracker#700`** — a real, but different and still-`OPEN` PR (title: "Confirm UMR-20260802-061754-a57f deregistered, PR 697 is sole Kernel-registration path", unrelated topic, submitted ~06:53 vs `d557`'s 12:11). Read PR #700's own body directly to confirm: it is genuinely about a different UMR pair (`061754-a57f`/`061325-aa9d`) from earlier the same day. So the dispatch-gateway's own duplicate-PR match for `d557` is spurious — the PR that actually delivered `d557`'s content is #717, verified by its own body text, not by the DB row. Flagging this as a real, live inaccuracy in the resource-governor's own bookkeeping rather than silently substituting the "obviously correct" PR without saying so.

### Live-state-drift correction to this matrix's own item 13

Item 13 above (still reads, unedited above per this file's "amend, don't rewrite" instruction) states: *"PR #692 is still OPEN, `mergeable: CONFLICTING`, never merged to `origin/main`."* **That is now stale.** Real, fresh check performed for this amendment (`gh pr view 692`, live): `state: MERGED`, `mergedAt: 2026-08-02T15:34:16Z`, merge commit `9edb6ed43f5f635143fa73844abf2da9aa4e9d5b`. `ai-os/EXISTING_MODULE_ENGINE_WIRING_MAP_2026-08-02.md` is now genuinely present on `main`. Item 13's "~40% production-live" figure and its "Production ready: No, blocker: PR #692 unmerged" line are both now out of date — this is flagged here rather than hand-edited into item 13's body, consistent with the "amend, do not rewrite existing rows" instruction for this pass; a full re-score of item 13's percentage is out of scope for this amendment (traceability/classification only, per SPEC).

### Real evidence (commit hashes, not narrated)

| PR | Merge commit | Merged at (UTC) | CI | Audit comment (final) |
|---|---|---|---|---|
| #716 | `9c56349cc34ec8409d882bc2d8ba81e7305a1fda` | 2026-08-02T14:19:42Z | Build/Unit Tests/E2E Tests/Guardrail Presence Check all `SUCCESS` | `AUDIT: PASS` (after 2 earlier `AUDIT: FAIL` cycles on this same PR — real iterative review, not self-certified) |
| #717 | `2a390fa2df99679c6287b2ba701ec976f7db5b1a` | 2026-08-02T14:00:13Z | all `SUCCESS` | `AUDIT: PASS` |
| #692 | `9edb6ed43f5f635143fa73844abf2da9aa4e9d5b` | 2026-08-02T15:34:16Z | all `SUCCESS` | `AUDIT: PASS` (after 1 earlier `AUDIT: FAIL`) |
| #720 | `993beb69eac1f42e4ffbbb51a5bd73d4a277d275` | 2026-08-02T15:40:22Z | all `SUCCESS` | `AUDIT: PASS` (independent audit from a separate session/task, per its own comment text) |
| #14 (veridian-scripts) | `09c5879961636134e19cd1f32b017170d884290e` | 2026-08-02T15:07:14Z | no CI configured on this repo's branch (`gh pr checks 14` → "no checks reported") — real test evidence is local (`pytest`, `py_compile`, live dry-run) per the PR body's own test-plan log, not a CI gate | `AUDIT: PASS` |
| #122 (claude-control) | `90f09d74985ea0e4dac0dee0747a742fa15f1e7d` | 2026-08-02T15:56:18Z | not checked (repo has no equivalent CI wiring confirmed this pass) | `AUDIT: PASS` |

### Dependency status

- #716 depends on `UMR-20260802-104058-25ba` (this same parent — satisfied by construction, matrix already exists as of #716's own merge).
- #717 depends on `UMR-20260802-054239-4251` (Kernel registration) — **satisfied**: that UMR is `status=completed` in the live DB, real merge of PR #697 (`99c2255f82e7a9e1961f4b5410a22e482913775e`), matches item 10/14 above.
- #692 depends on nothing upstream blocking it (parallel job, explicitly "does not block the master directive" per its own PR body) — its dependency chain to the master directive `UMR-20260802-034545-3388` is informational, not blocking; that master UMR is itself `status=rejected_duplicate` (cancelled by design, per its own `reason` field: "duplicate of the same master-directive text already executed directly by the interactive Claude Code CLI").
- #720 depends on `UMR-20260802-123246-f2e7` / `UMR-20260802-124023-371b` (both `status=running` in the live DB, but their real-world content — the Vercel domain investigation and matrix correction — is independently confirmed complete via live `curl`/API checks documented in item 12 above; the DB's `running` label was not treated as evidence per this matrix's own stated method).
- #14 depends on nothing upstream; PM decision `UMR-20260802-090702-c813` for the triage-escalation half is `status=running` in the DB but the real code is live on `origin/main` (confirmed via `git show origin/main:dispatch-tick.py` containing `find_stuck_tasks`/`write_stuck_tasks_heartbeat`).
- #122 depends on `UMR-20260802-080051-6e48` (`status=completed`, real) and `UMR-20260802-083104-5987` (`status=running` in DB, but its real output — the 9-entry reconciliation — is the content of #122 itself, now merged).

**Canonical artifact for this amendment**: `ai-os/IMPLEMENTATION_MATRIX_2026-08-02.md` (this file — amended in place, no new file created).
