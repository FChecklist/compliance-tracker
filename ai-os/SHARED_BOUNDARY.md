...
| `dpdp` | NO | YES | NO |
| `public` | NO (the only objects in public today are dpdp_* functions) | YES (dpdp_* RPCs) | NO |

R5. A PROJEXA SECURITY DEFINER function, if ever added, is never granted to `anon`. PROJEXA has 0 such functions today (see section 5, guard query G-4).
...
| 7 | Supabase Vault (`vault.secrets` on verdian-ai) | 2 secrets by name: `dpdp_timer_secret`, `dpdp_timer_url`, both created 2026-09-22 09:43:14 UTC. Nothing PROJEXA-owned (A09 s2). Values are never printed or copied into any document. | DPDP (both existing secrets) | DPDP | R1; PROJEXA adds a secret only through a claim |
...
Exact count today: 2. The PROJEXA project has no `cron` schema, so 0 jobs there.
