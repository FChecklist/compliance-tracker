...
Totals: 6 Edge Functions on verdian-ai (2 DPDP with traffic, 3 VERIDIAN AI-OS with 0 traffic in 7 days, 1 PROJEXA: projexa-timer, deployed 2026-09-25) and 2 on PROJEXA's own project (test tooling). PROJEXA has 1 runtime Edge Function today. The first PROJEXA runtime functions are planned by PMD-01 (the gateway, working name `projexa-read`) and PMD-12 (exchange-rate-refresh); each needs a claim first (R1). Addendum A1 said "5 running"; A09 shows ACTIVE means deployed, and 3 of 5 had 0 invocations in 7 days.
