...
| `dpdp` | NO | YES | NO |
| `public` | ONLY `projexa_timer_*` and `projexa_read_*` functions, granted to `service_role` alone (PMD-12 and PMD-01; the Edge Functions reach the database through PostgREST, which exposes only `public`), and, planned under U-46, the `ai_work_link_*` functions named in ACTIVE-CLAIMS (PMD-46: 23 for `service_role` alone and `ai_work_link_call_retention` for `postgres` alone), and, planned under U-40, `projexa_scheduler_bridge_check_bearer` and `projexa_scheduler_bridge_due_count` (`service_role` alone) | YES (dpdp_* RPCs) | NO |

R5. A PROJEXA SECURITY DEFINER function is never granted to `anon` or `authenticated`. PROJEXA has 4 such functions since 2026-09-25: `public.projexa_timer_check_bearer`, `public.projexa_timer_exchange_plan`, `public.projexa_timer_apply_exchange_rates` (migration 0615, U-21), all `service_role` only (guard query G-5 returns 0), and `platform.rpc_resolve_ai_link_scoped` (migration 0614, U-18b), executable by `app_runtime` alone exactly like the VERIDIAN resolver `platform.rpc_resolve_ai_link_token` it sits beside (guard query G-3 returns 0). Migration 0642 (U-40, planned) adds two more, `public.projexa_scheduler_bridge_check_bearer` and `public.projexa_scheduler_bridge_due_count`, both `service_role` only, so the count becomes 6 (guard query G-5 matches them by name and must still return 0).
...
| (planned) | cost001-fm-ppm-generate-occurrences | `0 2 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_fm_ppm_generate_occurrences()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0632 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-metric-alerts | `0 5 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_metric_alerts()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0633 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-the-firm-recur-engagements | `30 7 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_the_firm_recur_engagements()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0634 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-audit-cadence | `15 8 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_audit_cadence()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0635 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-task-nudge-digest | `0 8 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_task_nudge_digest()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0636 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-report-schedules | `5 * * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_report_schedules()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0637 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-orchestra-log-purge | `45 9 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_orchestra_log_purge()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0638 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-ai-reduction-snapshot | `0 2 1 * *` | not created yet | PROJEXA (cost) | `select compliance.cron_ai_reduction_snapshot()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0639 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-pipeline-stuck-deal-digest | `50 9 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_pipeline_stuck_deal_digest()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0640 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | cost001-crm-lead-followup-alerts | `15 11 * * *` | not created yet | PROJEXA (cost) | `select compliance.cron_crm_lead_followup_alerts()` as postgres, pure SQL, no HTTP; from supabase/prepared/cost001; migration 0641 (U-41 part B) | 0 (claimed 2026-09-26) | PLANNED (claim on main; row updated when created) |
| (planned) | projexa-scheduler-bridge | set when the job is created | not created yet | PROJEXA | `net.http_post` to Edge Function `projexa-scheduler-bridge`, url and bearer read from Vault like the other PROJEXA jobs | 0 (claimed 2026-09-25, U-40) | PLANNED (claim on main; row updated when created) |
...
| projexa-document-extract | verdian-ai | planned, not deployed yet | - | false | - | - | will be `supabase/functions/projexa-document-extract` | PROJEXA | Reads an uploaded workbook or document and returns a schema-validated BOQ or project record set; the extraction model call is made only here (E-13) and only through the provider the owner chooses (BR-509), so until then it answers 503 model_not_configured; claimed 2026-09-25 (U-36) |
| projexa-scheduler-bridge | verdian-ai | planned, not deployed yet | - | false | - | - | will be `supabase/functions/projexa-scheduler-bridge` | PROJEXA | Target of the pg_cron job of the same name (U-40): runs a due schedule as the schedule's owner and records a proposal (PMD-05); claimed 2026-09-25 (U-40) |
...
- 2026-09-26: planned rows added for the ten cost001-* cron jobs (U-41 part B); the scheduler bridge table compliance.pipeline_schedules, the two extraction objects and the Cloudflare Pages project are claimed in ACTIVE-CLAIMS (PHASE 5 ADDENDUM 2).
- 2026-09-26: R4 public row extended with the ai_work_link_* functions the U-46 claim already names (PMD-46).
- 2026-09-26: R5 gains two planned PROJEXA definer functions, public.projexa_scheduler_bridge_check_bearer and public.projexa_scheduler_bridge_due_count (migration 0642, U-40; service_role alone, guard G-5 matches them by name); R4 and PMD-46 corrected to the count of 23 service_role functions of the link.
