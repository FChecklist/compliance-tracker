...
R5. A PROJEXA SECURITY DEFINER function is never granted to `anon` or `authenticated`. PROJEXA has 0 such functions today; migration 0615 (U-21, claimed in ACTIVE-CLAIMS on 2026-09-25) adds 3 in schema `public`, all `service_role` only (see section 7, guard queries G-4 and G-5).
...
| 7 | Supabase Vault (`vault.secrets` on verdian-ai) | 2 secrets by name: `dpdp_timer_secret`, `dpdp_timer_url`, both created 2026-09-22 09:43:14 UTC. Nothing PROJEXA-owned yet (A09 s2); U-21 will add `projexa_timer_url` and `projexa_timer_secret` (claimed 2026-09-25; the secret value is generated inside the database). Values are never printed or copied into any document. | DPDP (the two existing secrets); PROJEXA (the two planned) | DPDP; PROJEXA for its own two | R1; PROJEXA adds a secret only through a claim |
...
Exact count today: 2 (a third, the PROJEXA job in the last row, is claimed and not yet created). The PROJEXA project has no `cron` schema, so 0 jobs there.
...
| (planned) | projexa-exchange-rate-refresh | `30 9 * * *` (daily, the slot of the Vercel cron it replaces) | not created yet | PROJEXA | `net.http_post` to Edge Function `projexa-timer` with body `{"job":"exchange_rate_refresh"}`, url and bearer read from Vault (`projexa_timer_url`, `projexa_timer_secret`); the function fetches open.er-api.com once per base currency and writes both directions per org through `projexa_timer_apply_exchange_rates` (same rows as `refreshLiveExchangeRatesForAllOrgs`) | 0 (claimed 2026-09-25, PMD-12) | PLANNED (claim on main; row updated when created) |
...
| (planned) projexa-timer | verdian-ai | not deployed yet | - | false | - | - | will be `supabase/functions/projexa-timer` | PROJEXA | Target of the PROJEXA cron job; bearer checked through a Vault RPC like dpdp-monday-email; claimed 2026-09-25 (PMD-12) |

Totals: 5 Edge Functions on verdian-ai (2 DPDP with traffic, 3 VERIDIAN AI-OS with 0 traffic in 7 days) and 2 on PROJEXA (test tooling). PROJEXA has 0 runtime Edge Functions today (A09 s1b). The first PROJEXA runtime functions are planned by PMD-01 (the gateway, working name `projexa-read`) and PMD-12 (exchange-rate-refresh); each needs a claim first (R1). Addendum A1 said "5 running"; A09 shows ACTIVE means deployed, and 3 of 5 had 0 invocations in 7 days.
