...
| C3 | #13 Vertical Scalability | Document the serverless resource-limit tradeoff + audit heaviest workloads. Docs+code. **RE-SCORED CLOSED 2026-07-26 (V2-12, PR TBD):** `ai-os/V2-12_SERVERLESS_RESOURCE_LIMITS.md` documents the live Vercel Hobby-plan limits (300s fixed max duration, 4.5 MB body cap, 2 GB memory — no `maxDuration`/`functions` override anywhere in this codebase today) and audits payroll/report/bulk-op routes plus upload routes against them, widened to every awaited-DB-call-in-a-loop pattern. Five HIGH-severity N+1 routes found (payroll run processing, bulk attendance marking, the compliance-item CSV importer — which also has no file-size cap at all, the cost-overrun report's ~500-project fan-out, and GST batch-confirm); a shared cheap batching fix is recommended as the first escalation for all five, with the cost-overrun report flagged as the one genuine queue/background-job candidate if that fix alone proves insufficient — a conditional, evidence-gated escalation, not a speculative build. | V2-12 ✅ |
