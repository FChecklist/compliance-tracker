...
$PROGRESS_INSTRUCTION"
  # Was a hardcoded literal ("quality gate auto-fix retry") identical for
  # every task fleet-wide, so check_existing_capability()'s system_index
  ...
  # the real failing gate name(s) instead. Matches the fix already live in
  # /opt/veridian/scripts/worker-entrypoint.sh (the actually-executed copy,
  # commit e1aa1f2) -- this mirror had drifted stale and was carrying the
  # pre-fix version.
