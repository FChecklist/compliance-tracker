...
| U-44 | Universal AI Work Link spec accepted after a non-author audit (AUDIT: PASS) | BR-228, BR-229 [BR-306] | Done. The DPDP link lookup goes by token hash only (PR #1901) and the PROJEXA M4 public token route is gone (projexa PR #325); BR-587 passes. The shared helper de-duplication is the recorded exception EXC-DUP-2 (PMD-48). |
...
| U-29 | Surface 1: AI-prepared approval page with an approve action | BR-409, BR-410, BR-424 [BR-501] | Done. Approvals page, approve action and the surface 1 route merged; BR-409, BR-410, BR-424 pass. |
...
| U-33 | Browser-first BOQ: offline load, Web Worker filter, 0 function invocations, CDN HIT | BR-419, BR-420, BR-421, BR-422, BR-423 [BR-501] | Done. Browser-first BOQ reads (projexa PR #323) and verify scripts (PR #1899); BR-419 passes. BR-420 and BR-421 run through the manual runner workflow projexa-boq-playwright.yml (the laptop has no RAM for Next dev plus Chromium). |
| U-46 | Link endpoints on Supabase Edge: Markdown manual, OpenAPI, MCP, REST; one project and one user | BR-424 [BR-523, BR-524] | Done. Migrations 0621 to 0628 live, the ai-work-link Edge function deployed 2026-09-26 (v1); BR-480 to BR-495, BR-499, BR-523 and BR-581 to BR-583 pass. BR-492 runs from a GitHub runner (workflow awl-live-rate-limits.yml). |
| U-47 | Paste-back fallback for AIs that cannot reach the URL (ties to surface 1) | BR-424 [BR-524] | Done. Paste-back route, the Level-2 confirm route and the two static pages (live at projexa-link-pages.pages.dev, OD-3); BR-424 and BR-496 pass. BR-497 needs two real sign-ins and BR-498 needs writes on. |
...
| U-36 | Extraction: extend document-extraction-service.ts (every xlsx sheet, BOQ/project schema, Edge model call, idempotent, schema-validated) | BR-505, BR-506, BR-507, BR-508, BR-509, BR-510, BR-511, BR-525 | Done. Extraction service, Edge function, injection defence and the spend cap (U-36b, PR #1898, BR-526) merged; BR-505 to BR-508 and BR-526 pass. The Edge function is not deployed yet: it needs the owner Groq secret (BR-509). |
| U-37 | POST projects/from-document reusing createProject() and createBoq() | BR-508, BR-509, BR-525 | Not started. |
| U-38 | Remaining registry entries incl. create_drawing and create_mom traps; billing claims read-only | BR-512, BR-513, BR-522, BR-525 | Done. Registry entries and traps merged; BR-512 and BR-513 pass. |
...
| U-40 | Scheduler bridge: pg_cron -> Edge -> runSubmission as the schedule owner | BR-511, BR-515, BR-516, BR-517, BR-525 | Done. Scheduler bridge route and the pg_cron job merged; BR-515 and BR-516 pass. The bridge Edge function needs the owner Vault and Edge secrets before it is deployed. |
| U-41 | Vercel crons: 1 in compliance-tracker, 0 in projexa, none */N; crons drift guard | BR-516, BR-518, BR-519 [BR-328, BR-329] | Done. One Vercel cron left, ten cost001 pg_cron jobs live; BR-518 and BR-519 pass. |
| U-42 | All 28 surface cells proven | BR-520, BR-521, BR-522 | Partly done. BR-525 passes (five planted defects, PR #1900). BR-520 and BR-522 need the link write path switched on (spike S-1 and the executor secret, owner). |
