...
- **Chief Audit Officer, division heads, and any judgment-critical auditor role**: GLM-5.2 (the roster's own established "primary lifting" tier) -- never the floor tier. **Corrected 2026-07-14**: this originally also permitted GPT-5.5 as an alternative for independence-from-the-primary-reviewer cases (mirroring `security_threat_analyst`'s "genuinely different vendor" reasoning). GPT-5.5 was removed from the judgment tier entirely per Owner directive that day (real cost concern plus a real safety concern: an unbounded GLM-5.2-escalates-to-gpt-5.5-for-cross-reference pattern left standing by default). GLM-5.2 is now the sole judgment-tier model, no exception -- see `ai-os/CONSTITUTION.yaml`'s `ai_orchestra_tiers.levels[TIER-3]`.
